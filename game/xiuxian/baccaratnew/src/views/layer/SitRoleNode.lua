...
function SitRoleNode:onSitDown( useritem, bAni, isBanker )
	...
	local head = PopupInfoHead:createNormalCircle(useritem, self.m_headSize - 10,("Circleframe.png"))
	if nil ~= head then
		--head:setPosition(15,10)
		self.m_csbNode:addChild(head)
		local size = cc.Director:getInstance():getWinSize()
		local pos = cc.p(size.width * 0.15, size.height * 0.33)
		local anchor = cc.p(self.m_nIndex / 8, 1.0)
		if self.m_nIndex > 4 then			
			pos = cc.p(size.width * 0.62, size.height * 0.33)
		end
	...
	--昵称
	self.m_clipNick:setString(useritem.dwGameID)
...
--[[	local str = ExternalFun.numberThousands(useritem.lScore)
	...
	local str = string.format("%.2f",useritem.lScore)
...
function SitRoleNode:gameEndScoreChange( useritem, changescore )
	...
	local str = "."..string.format("%.2f",changescore)
