...
function GameViewLayer:initCsbRes(  )
	...
	self:addToRootLayer(self.m_controlLayer, TAG_ZORDER.CONTROL_ZORDER)
...
function GameViewLayer:initUserInfo(  )	
	...
	local head = g_var(PopupInfoHead):createNormalCircle(self:getMeUserItem(), 86,("Circleframe.png"))
	head:setPosition(tmp:getPositionX() - 2,tmp:getPositionY())
...
function GameViewLayer:refreshJetton(  )
	...
	local str = string.format("%.2f",self.m_lHaveJetton)
	...
	for k,v in pairs(children) do
		...
		if remove then
			...
			elseif "other" == wchair then
				self:generateBetAnimtion(v, {x = winSize.width-60, y = 40}, count)
...
function GameViewLayer:onGetUserBet( )
	...
	if nil ~= sp and nil ~= btn then
		...
		if useritem ~= nil then
			self.m_controlLayer:setPlayerAreaBet(useritem.dwGameID,area,data.lAreaAllJetton,data.cbAndroidUser,data.lAllJetton)
			if data.cbAndroidUser == 0 then
				print("下注玩家------",useritem.dwGameID,area,data.lAreaAllJetton)
			end
		end
	end
...
function GameViewLayer:reEnterUserBet( cbArea, llScore )
	...
	----------------------------
	--self.m_controlLayer:setAreaTotalBet(cbArea,llScore,0)
end
--断线重连更新玩家总下注(控制端)
...
function GameViewLayer:gameDataInit( )
    ...
    --播放背景音乐
    ExternalFun.playBackgroudAudio("GAME_BLACKGROUND.mp3")
...
function GameViewLayer:getBetFromPos( wchair )
	...
	if self:isMeChair(wchair) then
		...
		if nil ~= tmp then
			...
			return {x = winSize.width-60, y = 40}
		end
	end
...
function GameViewLayer:updateClock(tag, left)
	...
	if self.m_controlLayer ~= nil then
		self.m_controlLayer:showLeftTime(str)
	end
	---------------------------------------------
