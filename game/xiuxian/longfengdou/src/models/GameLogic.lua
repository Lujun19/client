...
function GameLogic.GetCardValue( cbCardData )
	return bit.band(cbCardData, GameLogic.MASK_VALUE);
end

--获取花色
function GameLogic.GetCardColor( cbCardData )
	return bit.band(cbCardData, GameLogic.MASK_COLOR);
end
------------------------------------------------------------------
...
--[[	--变量定义
	...
	return GameLogic.GetCardValue(cbCardData[1])

--[[	--对子
	if GameLogic.GetCardValue(cbCardData[1]) == GameLogic.GetCardValue(cbCardData[2]) then	
		return 10;
	else
		local maxValue = 0;
		local cbPoint1 = GameLogic.GetCardValue(cbCardData[1]);
		local cbPoint2 = GameLogic.GetCardValue(cbCardData[2]);
		if cbPoint1 >= 10 then
			cbPoint1 = 0
		end
		if cbPoint2 >= 10 then
		...
		if maxValue == 0 then
			local LittleValue = 0;
			if GameLogic.GetCardValue(cbCardData[1]) > GameLogic.GetCardValue(cbCardData[2]) then
				LittleValue = GameLogic.GetCardValue(cbCardData[2])
			else
				LittleValue = GameLogic.GetCardValue(cbCardData[1])
			end
			--2+8最大
			if LittleValue ==2 then
				return 11;
			else
				return maxValue;
			end
