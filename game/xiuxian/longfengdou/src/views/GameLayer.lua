...
function GameLayer:onEventGameSceneFree( dataBuffer )
	...
	self._gameView.lBottomPourImpose = cmd_table.lBottomPourImpose
    --申请条件
...
function GameLayer:onEventGameSceneJetton( dataBuffer )
    ...
    for i=1,g_var(cmd).AREA_MAX do
		...
		self._gameView:showPlayerBet(i,ll,false)
		
		--机器人区域总下注
		ll = cmd_table.lAndroidBet[1][i];
		self._gameView:showPlayerBet(i,ll,true)
    end
	...
	for i=1,g_var(cmd).GAME_PLAYER do
		for j=1,g_var(cmd).AREA_MAX do		
			betScore = cmd_table.lPlayerAreaBet[1][i].AreaBet[1][j]
			gameid = cmd_table.dwGameID[1][i]
			totleBet = cmd_table.lPlayerTotleBet[1][i]
			self._gameView:showPlayerAreaBet(j,betScore,gameid,totleBet)
		end
	end
...
function GameLayer:onEventGameSceneEnd( dataBuffer )
    ...
    -- 刷新下注信息
    self._gameView:refreshJetton()
	self._gameView:enableJetton(false)
    ...
    self._gameView:onGetGameCard(tabRes, false, cmd_table.cbTimeLeave,cmd_table.cbWinArea[1])
...
function GameLayer:onSubGameStart( dataBuffer )
	...
	self._gameView.lBottomPourImpose = self.cmd_gamestart.lBottomPourImpose
	--设置退出按钮是否可用
	self._gameView:setExitEnabled(false)
end
...
function GameLayer:onSubGameEnd( dataBuffer )
    ...
    for i = 1, cmd_table.cbCardCount[1][1] do
        tabRes.m_idleCards[i] = cmd_table.cbTableCardArray[1][i]
		print("发牌的数据 龙",tabRes.m_idleCards[i])
    end
    for i = 1, cmd_table.cbCardCount[1][2] do
        tabRes.m_masterCards[i] = cmd_table.cbTableCardArray[2][i]
		print("发牌的数据 凤",tabRes.m_masterCards[i])
    end
...
function GameLayer:onSubSendRecord( dataBuffer )
	...
	self._gameView:updateGameRecordScene()
end
