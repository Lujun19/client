...
local Game_CMD = appdf.req(appdf.GAME_SRC.."xiuxian.longfengdou.src.models.CMD_Game")
...
function ControlLayer:ctor( viewParent )
	...
	for i=1,5 do
		...
		self.m_btnControlTimes[i]:setTag(i+3)
...
function ControlLayer:OnAddpeizhi(dwGameID,score)	
	...
	for i = 1,lItemCount  do
		...
		if lItem ~= nil then
			...
			lGameID  = tonumber(lItem:getChildByName("Text_gameid"):getString())
			if dwGameID == lGameID then
				lItem:getChildByName("Text_score"):setString(ExternalFun.formatScoreText(score))
				bOK = true
				break
			end
		end
	...
	if bOK == false then
		...
		item:getChildByName("Text_score"):setString(ExternalFun.formatScoreText(score))
...
function ControlLayer:showPlayerAreaBet(index,arealScore,dwGameID,totleBet)
	...
	if nil ~= item then
		...
		local TableBetSocre = item:getChildByName("Text_usertotleBet")
...
function ControlLayer:cleanAreaBet()
	...
	for i = 1,lItemCount  do
		...
		if lItem ~= nil then
			...
			local totlebet = lItem:getChildByName("Text_usertotleBet")
			totlebet:setString(0)	
		end
	end
...
function ControlLayer:setPlayerAlljettion(dwGameID,lTdTotalScore,lTotalScore)
	...
	if (nil ~= item) then
		...
		if lTdTotalScore  < 0 then
			...
			local ltdTotalScore = item:getChildByName("Text_winscoreToday")
		...
		if lTotalScore  < 0 then
			...
			strlTdTotalScore = "-"..strlTdTotalScore
			local ltdTotalScore = item:getChildByName("Text_winscoreAll")
			ltdTotalScore:setString(strlTdTotalScore)
		else
			local ltotalScore = item:getChildByName("Text_winscoreAll")
...
function ControlLayer:UptdwinscoreItemList( )
	...
	local lItemCount = table.maxn(self.m_areaBetListView:getItems()) 
	local pzItemCount = table.maxn(self.m_addPlayerListView:getItems())
	local ncount = 0
	for i = 1,lItemCount  do
		local lItem = self.m_areaBetListView:getItem(i-1)
		if lItem ~= nil then
			...
			for j =1,pzItemCount do
				local pzItem = self.m_addPlayerListView:getItem(i-1)
				if pzItem ~= nil then
					local lpzGameID = nil
					lpzGameID  = tonumber(pzItem:getChildByName("Text_gameid"):getString())
...
function ControlLayer:setPlayerEnter(gameID,cbAndroidUser)
	...
	if (nil == item) then
		...
		local totlebet = item:getChildByName("Text_usertotleBet")
		totlebet:setString(0)
...
function ControlLayer:setPlayerAreaBet(gameID,cbArea,lScore,cbAndroidUser,lAllJetton)
	...
	if (nil == item) then
		...
		areaBet[cbArea]:setString(ExternalFun.formatScoreText(lScore))
		
		local totlebet = item:getChildByName("Text_usertotleBet")
		totlebet:setString(ExternalFun.formatScoreText(lAllJetton))
		...
		areaBet:setString(ExternalFun.formatScoreText(lScore))
