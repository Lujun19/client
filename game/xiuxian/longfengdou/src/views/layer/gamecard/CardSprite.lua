...
--[[	local sp = CardSprite.new();
	...
	if nil ~= sp and nil ~= tex and sp:initWithTexture(tex, tex:getContentSize()) then
		...
		sp.m_cardValue = yl.POKER_VALUE[cbCardData] --math.mod(cbCardData, 16)--bit.band(cbCardData, 0x0F)
		sp.m_cardColor = yl.CARD_COLOR[cbCardData] --math.floor(cbCardData / 16)--bit.rshift(bit.band(cbCardData, 0xF0), 4)
...
--[[	local sp_card = nil;
	local str = ""
	str = string.format("dragon_card_type_%d.png",yl.POKER_VALUE[cbCardData])
	local frame = cc.SpriteFrameCache:getInstance():getSpriteFrame(str)
	if nil ~= frame then
		sp_card = cc.Sprite:createWithSpriteFrame(frame);
		self:addChild(sp_card);
	end
...
function CardSprite:setCardValue( cbCardData )
	...
	self.m_cardValue = yl.POKER_VALUE[cbCardData] --math.mod(cbCardData, 16) --bit.b(cbCardData, 0x0F)
