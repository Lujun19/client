...
function ApplyListLayer:ctor( viewParent)
	...
	--上庄条件
	self.m_bankerConditon = sp_bg:getChildByName("BankerCondition_text")
	self.m_bankerConditon:setString("")
	
	content:removeFromParent()
end
function ApplyListLayer:setBankerCondition(lConditon)
	
	--local str = ExternalFun.formatScoreText(lConditon);
	local str = string.format("%.2f",lConditon)
	self.m_bankerConditon:setString("上庄条件："..str);
end
...
function ApplyListLayer:onButtonClickedEvent( tag, sender )
	...
	if ApplyListLayer.BT_CLOSE == tag then
		...
		local moveto = cc.MoveTo:create(0.5,cc.p(0,winsize.height))
