...
local DEFAULT_BET = 1
--筹码运行时间
local BET_ANITIME = 0.4

--下注数值
GameViewLayer.m_BTJettonScore = {1, 5, 10, 50, 100}

--下注值对应金币个数
GameViewLayer.m_JettonGoldBaseNum = {1, 2, 3, 4, 5}

--区域倍数
GameViewLayer.m_multiply = {g_var(cmd).MULTIPLE_LONG,g_var(cmd).MULTIPLE_PING,g_var(cmd).MULTIPLE_FENG}
...
function GameViewLayer:initCsbRes(  )
	...
	--动画层
	self.m_animateNode = ExternalFun.loadCSB("animate/animate_lfd.csb", self);
	self.m_jettonAnimate = ExternalFun.loadTimeLine("animate/animate_lfd.csb")
	self.m_animateNode:setVisible(false)
	self.m_animateNode:runAction(self.m_jettonAnimate)
	self.m_animateNode:setLocalZOrder(10)
	
	local btnLightNode = self.m_animateNode:getChildByName("btnLight")
	self.m_btnLightNode = btnLightNode
	
	local lightflynode = self.m_animateNode:getChildByName("lightfly")
	self.m_lightflynode = lightflynode
	self.m_lightflynode:setVisible(false)
	--初始化按钮
...
function GameViewLayer:initGameRecord( csbNode )
	...
	local str = ""
	for i=1,35 do
		str = string.format("Sprite_record_%d", i)
		self.m_recordSp[i] = self.m_recordBg:getChildByName(str)
		self.m_recordSp[i]:setVisible(false)
		self.m_recordPos[i] = cc.p(self.m_recordSp[i]:getPositionX(),self.m_recordSp[i]:getPositionY())
	end
...
function GameViewLayer:updateGameRecord( )
	...
	if len<=35 then
		for i = index,len do
			...
			self.m_recordSp[nCount]:setVisible(true)
			
			if g_var(cmd).AREA_LONG == rec.m_cbGameResult then
				str = "rec_long.png"
			elseif g_var(cmd).AREA_FENG == rec.m_cbGameResult then
				str = "rec_feng.png"
			...
			if nil ~= frame then			
				self.m_recordSp[nCount]:setSpriteFrame(frame)
			end
	...
	local call1 = cc.CallFunc:create(function()
		...
		for i = index-1,len-1 do
			...
			self.m_recordSp[num]:setVisible(true)
			...
			local frame = cc.SpriteFrameCache:getInstance():getSpriteFrame(str)
			if nil ~= frame then			
				self.m_recordSp[num]:setSpriteFrame(frame)
			end
					
			num = num+1
		end
			
		self.m_recordSp[1]:setVisible(false)
		
		for i = 2,35 do
			self.m_recordSp[i]:runAction(cc.MoveTo:create(0.5,cc.p(self.m_recordPos[i-1].x,self.m_recordPos[i-1].y)))
		end
	end)
	local call2 = cc.CallFunc:create(function()
		...
		for i = index,len do
			...
			self.m_recordSp[nn]:setVisible(true)
			...
			local frame = cc.SpriteFrameCache:getInstance():getSpriteFrame(str)
			if nil ~= frame then			
				self.m_recordSp[nn]:setSpriteFrame(frame)
			end
			self.m_recordSp[nn]:setPosition(self.m_recordPos[nn].x,self.m_recordPos[nn].y)
			
			nn = nn+1
		end
	end)
	if len>35 then
		self:runAction(cc.Sequence:create(call1,cc.DelayTime:create(0.5),call2))
		local rec = vec[len]
		if g_var(cmd).AREA_LONG == rec.m_cbGameResult then
			str = "fly_left"
		elseif g_var(cmd).AREA_FENG == rec.m_cbGameResult then
		...
		self.m_animateNode:setVisible(true)
		self.m_btnLightNode:setVisible(false)
		self.m_lightflynode:setVisible(true)
		self.m_jettonAnimate:play(str,false)
	end
...
--动态加入更新游戏记录
function GameViewLayer:updateGameRecordScene( )
	local vec = self:getDataMgr():getRecords()
	local len = #vec
	local nCount = 1
	local index = 1
...
--[[	local btn = btn_list:getChildByName("bank_btn");
	...
	btn:addTouchEventListener(btnEvent); 
	--充值按钮
	btn = csbNode:getChildByName("Button_shop");
	btn:setVisible(false)
	------
	--上庄、抢庄
...
function GameViewLayer:initBankerInfo( ... )
	...
	self.m_systemHeadSp = banker_bg:getChildByName("System_head");
	
	self.m_systemHeadSp:setVisible(false)
	--庄家头像
	local headframe = banker_bg:getChildByName("Image_headframe")
	self.m_bankFrame = headframe
	--系统坐庄
	if self.m_wBankerUser == yl.INVALID_CHAIR then
		self.m_systemHeadSp:setVisible(true)
		local frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("man_0.png")
		if nil ~= frame then
			self.m_systemHeadSp:setSpriteFrame(frame)
			self.m_systemHeadSp:setScale(0.3)
		end
	else
		local bankeritem = self:getDataMgr():getChairUserList()[self.m_wBankerUser + 1];
		if bankeritem ~= nil then
			local head = g_var(PopupInfoHead):createNormal(bankeritem, 70)
			head:setPosition(headframe:getPositionX(),headframe:getPositionY())
			self.m_spBankerBg:addChild(head)
			local anchor = cc.p(0,1)
			local pos = cc.p(headframe:getPositionX(),headframe:getPositionY()-150)
			pos = headframe:convertToWorldSpace(pos)
			head:enableInfoPop(true,pos,anchor)
			self.m_bankerHead = head
		end
	end
end
...
function GameViewLayer:initUserInfo(  )	
	...
	local head = g_var(PopupInfoHead):createNormal(self:getMeUserItem(), 60)
	head:setPosition(tmp:getPositionX(),tmp:getPositionY())
	self.m_spBottom:addChild(head)
	local pos = cc.p(tmp:getPositionX(),tmp:getPositionY()+50)
...
function GameViewLayer:reSetUserInfo(  )
	...
	--local str = ExternalFun.formatScoreText(self.m_scoreUser);
	local str = string.format("%.2f",self.m_scoreUser)
	self.m_textUserCoint:setString(str);
	
	self.m_textUserID:setString("ID:"..myUser.dwGameID);
end
...
--[[	if nil ~= idx and nil ~= self.m_tabJettonAnimate[idx] then
	...
	if nil ~= idx and nil ~= self.m_jettonAnimate then
		local enable = lCondition >= self.m_pJettonNumber[idx].k;
		if enable then
			self.m_tabJettonAnimate[idx]:setVisible(true)
			self.m_animateNode:setVisible(true)
			self.m_lightflynode:setVisible(false)
			self.m_btnLightNode:setVisible(true)
			self.m_jettonAnimate:play("light",true)
			self.m_btnLightNode:setPosition(cc.p(self.m_tableJettonBtn[idx]:getPositionX(),self.m_tableJettonBtn[idx]:getPositionY()+3))
		end		
	end
	--防止选中下注筹码开牌阶段还会闪
...
function GameViewLayer:goldAnimation( ) 
	...
	--赢的区域的下注
	--local jettonscore = self.m_tableJettonNode[winarea].m_llAreaTotal
	--金币从庄家处飞到赢的区域
	local call1 = cc.CallFunc:create(function()
		self:showGoldToWinArea(winarea)
	end)
	local meChair =  self:getMeUserItem().wChairID
	--金币从赢的区域飞到玩家处/从输的区域飞到庄家处
	local call2 = cc.CallFunc:create(function()
		self:showGoldToPlayer(winarea)
		...
		for i = 1, g_var(cmd).MAX_OCCUPY_SEAT_COUNT do
			if nil ~= self.m_rankTopUser[i] and yl.INVALID_CHAIR ~= self.m_rankTopUser[i] then
				--非自己
	...
	local seq = cc.Sequence:create(call1, cc.DelayTime:create(1.0), call2)
	self:stopAllActions()
	self:runAction(seq)	
end
--庄家吐金币到赢的区域
function GameViewLayer:showGoldToWinArea(winArea)
	if winArea<=0 then
		return
	end
	local goldNum = #self.m_goldList[winArea]
	if goldNum == 0 then
		return
	end
	
    local childrenbet = self.m_betAreaLayout:getChildren()
      
	local btn = self.m_tableJettonArea[winArea];
	if goldNum>1 then
		goldNum = 1
	end
    --for i=1,goldNum do
    --    local name = string.format("%dbet%d", self.iBetTime,i) 
    --    local oldsp = self.m_betAreaLayout:getChildByName(name)
    --    self.m_betAreaLayout:removeChild(oldsp)
    --end
	for i=1,goldNum do
		local sp = nil
		local frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("table_chip.png")
		if nil ~= frame then
		...
		if nil ~= sp and nil ~= btn then
			...
			--筹码飞行起点位置
			local pos = cc.p(self.m_bankFrame:getPositionX(),self.m_bankFrame:getPositionY())
			pos = self.m_bankFrame:convertToWorldSpace(pos)
			sp:setPosition(pos)
			
			local endpos = self:getBetRandomPos(btn,winArea)
			local moveact = cc.MoveTo:create(BET_ANITIME, cc.p(endpos.x, endpos.y))
			local act = cc.EaseIn:create(moveact, 1)
			
			sp:stopAllActions()
			sp:runAction(cc.Sequence:create(cc.DelayTime:create(0.02 * i),act))
 
   
			self.m_betAreaLayout:addChild(sp)
			
			table.insert(self.m_goldList[winArea],sp)
		end
	end
    self.iBetTime = self.iBetTime + 1
    if self.iBetTime > 3 then
        self.iBetTime = 0
    end
end
function GameViewLayer:showGoldToPlayer(winarea)
	local goldNum = {}
	for i=1, 3 do
		goldNum[i] = #self.m_goldList[i]
	end
	if goldNum[1]==0 and goldNum[2]==0 and goldNum[3]==0 then
		return
	end
	...
	for k,v in pairs(tmp) do
		if nil ~= v then			
			if v:getTag() == winarea then
				local endpos = cc.p(winSize.width-100, 45)
				local moveact = cc.MoveTo:create(0.3, cc.p(endpos.x, endpos.y))
				local act = cc.EaseIn:create(moveact, 1)
				ExternalFun.playSoundEffect("bet.mp3")
				v:stopAllActions()
				v:runAction(cc.Sequence:create(cc.DelayTime:create(0.01 * count1),act,cc.CallFunc:create(function()
					v:removeFromParent()
				end)))
				count1 = count1 + 1
			else
				local endpos = cc.p(self.m_bankFrame:getPositionX(), self.m_bankFrame:getPositionY())
				endpos = self.m_bankFrame:convertToWorldSpace(endpos)
				local moveact = cc.MoveTo:create(0.3, cc.p(endpos.x, endpos.y))
				local act = cc.EaseIn:create(moveact, 1)
				ExternalFun.playSoundEffect("bet.mp3")
				v:stopAllActions()
				v:runAction(cc.Sequence:create(cc.DelayTime:create(0.01 * count2),act,cc.CallFunc:create(function()
					v:removeFromParent()
				end)))
			end
			count2 = count2 + 1
	...
	for k,v in pairs(children) do
		...
		if remove then
			...
			elseif "other" == wchair then
				self:generateBetAnimtion(v, {x = winSize.width-100, y = 45}, count)
...
function GameViewLayer:generateBetAnimtion( bet, pos, count)
	...
	ExternalFun.playSoundEffect("chip_fly.wav")
end
...
function GameViewLayer:updateRankSitUser()
	...
	for i=1, g_var(cmd).MAX_OCCUPY_SEAT_COUNT do
		...
		if item ~= nil then
			...
			local head = g_var(PopupInfoHead):createNormal(item, 70)
			if head ~= nil then
				head:setPosition(tmp:getPositionX(),tmp:getPositionY())
			...
			local gameid = self.m_rankTopNode[i]:getChildByName("Text_score")
			local szAddress = self.m_rankTopNode[i]:getChildByName("Text_name")
			local strid = item.dwGameID
			gameid:setString("ID:"..strid)
...
function GameViewLayer:onButtonClickedEvent(tag,ref)
	...
	elseif tag == TAG_ENUM.BT_USERLIST then
		...
		self.m_userListLayer:refreshList(userList,self.m_rankTopUser)
	...
	elseif tag == TAG_ENUM.BT_APPLYLIST then
		...
		self.m_applyListLayer:setPosition(cc.p(0,winsize.height))
		local moveto = cc.MoveTo:create(0.5,cc.p(0,0))
		self.m_applyListLayer:runAction(moveto)
	...
	elseif tag == TAG_ENUM.BT_CHECK then
		self:onCheckBoxClickEvent()
	else
...
function GameViewLayer:onCheckBoxClickEvent( )
	ExternalFun.playClickEffect()
	self.m_btnList:stopAllActions();
	self.m_btnList:setVisible(true)
	local actdown = cc.MoveBy:create(0.2,cc.p(0,-170))
	self.m_btnList:runAction(actdown);
...
function GameViewLayer:onGameStart( )
	...
	--更新玩家今日输赢值顺序
	self.m_controlLayer:UptdwinscoreItemList()
end
...
function GameViewLayer:onChangeBanker( wBankerUser, lBankerScore, bEnableSysBanker )
	...
	self.m_systemHeadSp:setVisible(false)
	if self.m_bankerHead ~= nil then
		self.m_bankerHead:removeFromParent()
		self.m_bankerHead = nil
	end
	local headframe = self.m_spBankerBg:getChildByName("Image_headframe")
	...
	if true == bEnableSysBanker then --允许系统坐庄
		if yl.INVALID_CHAIR == wBankerUser then
			nickstr = "系统坐庄"
			self.m_systemHeadSp:setVisible(true)
			local frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("Avatar0.png")
			if nil ~= frame then
				self.m_systemHeadSp:setSpriteFrame(frame)
			end
		else
			...
			if nil ~= userItem then
				...
				local head = g_var(PopupInfoHead):createNormal(userItem, 70)
				head:setPosition(headframe:getPositionX(),headframe:getPositionY())
				self.m_spBankerBg:addChild(head)
				local anchor = cc.p(0,1)
				local pos = cc.p(headframe:getPositionX(),headframe:getPositionY()-150)
	...
	if yl.INVALID_CHAIR == wBankerUser then
		self.m_textBankerCoin:setString("0.00");
	else
		self.m_textBankerCoin:setString(str);
	end
...
--[[	local nIdx = self:getJettonIdx(data.lBetScore);
	...
	local goldNum = self:getGoldNum(llScore)
    if goldNum > 1 then
        goldNum = 1
    end 
    --for i=1,goldNum do
	...
	for i=1,goldNum do
		...
		if nil ~= sp and nil ~= btn then
			...
			sp:setPosition(pos)
			--筹码飞行动画
			local act = self:getBetAnimation(self:getBetRandomPos(btn,area))
			sp:stopAllActions()
			sp:runAction(cc.Sequence:create(cc.DelayTime:create(0.02 * i),act))
 
			self.m_betAreaLayout:addChild(sp)
			
			table.insert(self.m_goldList[area],sp)
		end
	end
    self.iBetTime = self.iBetTime + 1
    if self.iBetTime > 3 then
	...
	if nil == self.m_tableJettonNode[area] then
		...
		jettonNode:setTag(-1);	
		if area == 2 then
			local pos1 = cc.p(jettonNode.m_textMyJetton:getPositionX(),jettonNode.m_textMyJetton:getPositionY())
			local pos2 = cc.p(jettonNode.m_textTotalJetton:getPositionX(),jettonNode.m_textTotalJetton:getPositionY())	
			pos1 = self.m_tagControl:convertToWorldSpace(pos1)
			pos2 = self.m_tagControl:convertToWorldSpace(pos2)
			jettonNode.m_textMyJetton:setPosition(pos1.x,pos1.y+113);
			jettonNode.m_textTotalJetton:setPosition(pos2.x,pos2.y-113);
		end
	...
	------------------------
	self.m_controlLayer:setAreaTotalBet(area,llScore,data.cbAndroidUser)
	local useritem = self:getDataMgr():getChairUserList()[wUser + 1]
	if useritem ~= nil then
		self.m_controlLayer:setPlayerAreaBet(useritem.dwGameID,area,lPlayerAreaBet,data.cbAndroidUser,data.lAllJetton)
	end
...
function GameViewLayer:reEnterGameBet( cbArea, llScore )
	...
	for k,v in pairs(vec) do
		local info = v;
        local goldNum = info.m_cbCount
        if goldNum > 1 then
            goldNum = 1
        end 
		for i=1,goldNum do
			--local str = string.format("table_chip_%d.png", info.m_cbIdx);
			local sp = cc.Sprite:createWithSpriteFrameName("table_chip.png");
			if nil ~= sp then
				--sp:setScale(0.35);
				sp:setTag(cbArea);
				local name = string.format("%dbet%d",self.iBetTime, i) --ExternalFun.formatScore(info.m_llScore);
				sp:setName(name);
				...
				table.insert(self.m_goldList[cbArea],sp)
			end
		end
        self.iBetTime = self.iBetTime + 1
        if self.iBetTime > 3 then
...
--更新扑克牌
function GameViewLayer:onGetGameCard( tabRes, bAni, cbTime,cbWinArea )
	if nil == self.m_cardLayer then
	...
	self.m_cardLayer:showCards(tabRes, bAni, cbTime,cbWinArea)
...
function GameViewLayer:gameDataInit( )
	...
	self.m_bankerHead = nil
	--系统坐庄头像图片
	self.m_systemHeadSp = nil
	--选中按钮发光
	self.m_btnLightNode = nil
	--金币列表
	self.m_goldList = {{},{},{}}
...
function GameViewLayer:onEventTouchCallback(eventType, x, y)
	if eventType == "ended" then
		local pos = cc.p(x,y)
		local rectBg = self.m_btnList:getBoundingBox()
        if not cc.rectContainsPoint(rectBg, pos) and self.m_btnList:isVisible() then
			self.m_btnList:stopAllActions();
			local actup = cc.MoveBy:create(0.2,cc.p(0,170))
			self.m_btnList:runAction(cc.Sequence:create(actup,cc.CallFunc:create(function()
				self.m_btnList:setVisible(false)
			end)));
...
function GameViewLayer:getBetFromPos( wchair )
	...
	if self:isMeChair(wchair) then
		...
		if nil ~= tmp then
			...
			return {x = winSize.width-100, y = 45}
		end
	end
...
function GameViewLayer:getBetAnimation( pos, call_back )
	local moveTo = cc.MoveTo:create(0.4, cc.p(pos.x, pos.y))
	if nil ~= call_back then
		return cc.Sequence:create(cc.EaseIn:create(moveTo, 1), call_back)
	else
		return cc.EaseIn:create(moveTo, 1)
	end
end
--获取移动动画
...
function GameViewLayer:getBetRandomPos(nodeArea,cbArea)
	...
	local nodeSize = cc.size(nodeArea:getContentSize().width - 80, nodeArea:getContentSize().height - 120);
...
--[[	if cbArea == 1 or cbArea == 3 then
		nodeSize.height = nodeSize.height - 70
	end--]]
...
function GameViewLayer:createClockNode()
	...
	self.m_pClock:setPosition(665,115)
...
--[[	self.m_clockBg = csbNode:getChildByName("sp_time_bg_1")
	...
	--倒计时
	self.m_pClock.m_atlasTimer = csbNode:getChildByName("Text_time")
	self.m_pClock.m_atlasTimer:setString("")

	--提示
	self.m_pClock.m_spTip = csbNode:getChildByName("Text_tip")
	self.m_pClock.m_spTip:setString("")
...
--[[	local str = string.format("sp_tip_%d.png", tag)
	...
	if g_var(cmd).kGAMEFREE_COUNTDOWN == tag then
		self.m_pClock.m_spTip:setString("空闲中：")
	elseif g_var(cmd).kGAMEPLAY_COUNTDOWN == tag then
		self.m_pClock.m_spTip:setString("下注中：")		
	elseif g_var(cmd).kGAMEOVER_COUNTDOWN == tag then
		self.m_pClock.m_spTip:setString("开奖中：")
	end
