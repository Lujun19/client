...
function GameViewLayer:initCsbRes(  )
	...
	--动画层
	self.m_animateNode = ExternalFun.loadRootCSB("animate/animate_zfb.csb", self);
	self.m_startBetAni = ExternalFun.loadTimeLine("animate/animate_zfb.csb")
	self.m_animateNode:setVisible(false)
	self.m_animateNode:runAction(self.m_startBetAni)
...
function GameViewLayer:initGameRecord( csbNode )
	...
	local str2 = ""
	for i=1,7 do
		str1 = string.format("Sprite_zhong_%d", i)
		self.m_recordZhong[i] = self.m_recordBg:getChildByName(str1)
		self.m_pointZhong[i] = self.m_recordZhong[i]:getChildByName("result_FontLabel")
		self.m_recordZhong[i]:setVisible(false)
		
		str2 = string.format("Sprite_bai_%d", i)
		self.m_recordBai[i] = self.m_recordBg:getChildByName(str2)
		self.m_pointBai[i] = self.m_recordBai[i]:getChildByName("result_FontLabel")
		self.m_recordBai[i]:setVisible(false)
	end
...
function GameViewLayer:updateGameRecord( )
	...
	for i = index,len do
		local rec = vec[i]
		self.m_newRecordSymbol:setVisible(true)
		self.m_recordZhong[nCount]:setVisible(true)
		self.m_recordBai[nCount]:setVisible(true)
		if len<8 then
			self.m_newRecordSymbol:setPosition(cc.p(self.m_recordZhong[len]:getPositionX(),self.m_recordZhong[len]:getPositionY()-17))
		else
			self.m_newRecordSymbol:setPosition(self.m_symbolPos)
		end
		if g_var(cmd).AREA_ZHONG == rec.m_cbGameResult then
			str1 = "charts_red.png"
			str2 = "charts_gray.png"
			str3 = "font/dragon_wan_win_fnt.fnt"
			str4 = "font/dragon_wan_lose_fnt.fnt"
		elseif g_var(cmd).AREA_BAI == rec.m_cbGameResult then
			str1 = "charts_gray.png"
			str2 = "charts_red.png"
			str3 = "font/dragon_wan_lose_fnt.fnt"
			str4 = "font/dragon_wan_win_fnt.fnt"
		elseif g_var(cmd).AREA_PING == rec.m_cbGameResult then
			str1 = "charts_blue.png"
			str2 = "charts_blue.png"
			str3 = "font/dragon_wan_equal_fnt.fnt"
			str4 = "font/dragon_wan_equal_fnt.fnt"
		end
		...
		if nil ~= frame1 then			
			self.m_recordZhong[nCount]:setSpriteFrame(frame1)
		end
		if nil ~= frame2 then			
			self.m_recordBai[nCount]:setSpriteFrame(frame2)
		end
		
		self.m_pointZhong[nCount]:setFntFile(str3)
		self.m_pointBai[nCount]:setFntFile(str4)
			
		local zhonePoint = rec.m_pServerRecord.cbPlayerCount
		local baiPoint = rec.m_pServerRecord.cbBankerCount
		if zhonePoint<10 then
			self.m_pointZhong[nCount]:setString(zhonePoint)
		elseif zhonePoint == 10 then
			self.m_pointZhong[nCount]:setString("豹")
		elseif zhonePoint == 11 then
			self.m_pointZhong[nCount]:setString("杠")
		end
		if baiPoint<10 then
			self.m_pointBai[nCount]:setString(baiPoint)
		elseif baiPoint == 10 then
			self.m_pointBai[nCount]:setString("豹")
		elseif baiPoint == 11 then
			self.m_pointBai[nCount]:setString("杠")
		end
...
function GameViewLayer:initJettonArea( csbNode )
	...
	local text = ccui.Text:create("本次下注为:", "base/fonts/round_body.ttf", 20)
...
function GameViewLayer:updateRankSitUser()
	...
	for i=1, g_var(cmd).MAX_OCCUPY_SEAT_COUNT do
		...
		if item ~= nil then
			...
			if string.len(strName) > 15 then
				strName = string.sub(strName,1,15)
			end
			nickName:setString("ID:"..strName)
			--local str = ExternalFun.formatScoreText(item.lScore)
			local strAddress = item.szAdressLocation
--[[			if string.len(strAddress) > 15 then
				strAddress = string.sub(strAddress,1,15)
			end--]]
...
--[[		for i = 1, g_var(cmd).MAX_OCCUPY_SEAT_COUNT do
		...
		if nil == self.m_tableJettonNode[area] then
			...
			if area>3 and area<16 then 
				jettonNode:setPosition(cc.p(btn:getPositionX(),btn:getPositionY()+40));
			elseif area==2 then
				jettonNode:setPosition(cc.p(btn:getPositionX(),btn:getPositionY()-60));
			elseif area==1 or area==3 then
				jettonNode:setPosition(cc.p(btn:getPositionX(),btn:getPositionY()-110));
			end
...
function GameViewLayer:reEnterGameBet( cbArea, llScore )
	...
	if nil == self.m_tableJettonNode[cbArea] then
		local jettonNode = self:createJettonNode()
		if cbArea>3 and cbArea<16 then 
			jettonNode:setPosition(cc.p(btn:getPositionX(),btn:getPositionY()+40));
		elseif cbArea==2 then
			jettonNode:setPosition(cc.p(btn:getPositionX(),btn:getPositionY()-60));
		elseif cbArea==1 or cbArea==3 then
			jettonNode:setPosition(cc.p(btn:getPositionX(),btn:getPositionY()-110));
		end
...
--[[		local userList = self:getDataMgr():getUserList()
		local quewangBgRect = self.m_queWangBg:getBoundingBox()
		pos = self.m_queWangUser:convertToNodeSpace(pos)
		if cc.rectContainsPoint(quewangBgRect, pos) then
			if self.m_gameQueWangLayer ~= nil then
				self.m_gameQueWangLayer:setVisible(true)		
				self.m_gameQueWangLayer:refreshRewardRank(userList,self.m_lHaveJetton)
			end	
        end--]]
	end
	return true	
...
function GameViewLayer:updateClock(tag, left)
	...
	elseif g_var(cmd).kGAMEPLAY_COUNTDOWN == tag then
		...
		if left == 3 then	
			self.m_animateNode:setVisible(true)		
			self.m_startBetAni:play("timeAnim",false)
		end
		if left <= 3 then
...
function GameViewLayer:OnUpBetInfo(cmd_table)
	local lPlayerBet = 0
	local lAnroidBet = 0
	local lPlayerAreaBet = 0
	for i=1,g_var(cmd).AREA_MAX do
		lPlayerBet = cmd_table.lPlayerBet[1][i]
		lAndroidBet = cmd_table.lAndroidBet[1][i]
		lPlayerAreaBet = cmd_table.lPlayerAreaBet[1][i]
		self.m_controlLayer:UpBetInfo(i,lPlayerBet,lAndroidBet,lPlayerAreaBet,cmd_table.dwGameID,cmd_table.lPlayerTotleBet)
	end
