...
local res_path = "game/xiuxian/shuiguolaba/res/"
	...
	local function imageLoaded(texture)--texture
        ...
        if m_nImageOffset == totalSource then
			...
			if PreLoading.bFishData  then
				...
				layer:stopAllActions()
                layer:removeFromParent()
	--			layer:runAction(cc.Sequence:create(callfunc1,cc.DelayTime:create(1.5),callfunc2,cc.DelayTime:create(1.5),callfunc3,cc.DelayTime:create(1.5),callfunc4))
			end
        	print("资源加载完成")
        end
    end
...
function PreLoading.GameLoadingView()     --跳转界面的ui
	local scene = cc.Director:getInstance():getRunningScene()
