...
function BankerList:InitData()
	...
	self.m_pTableView = cc.TableView:create(cc.size(600, 200))
	self.m_pTableView:setDirection(cc.SCROLLVIEW_DIRECTION_VERTICAL)
	self.m_pTableView:setPosition(cc.p(0,100))
...
--[[    local close = ccui.Button:create("game_res/anniu25.png","game_res/anniu26.png")
    ...
    close:setPosition(cc.p(527 - 5,411 - 5))
...
function BankerList:tableCellAtIndex( view, idx )
	...
	if nil == cell then
		...
		cellBG:setTag(1)
		cellBG:setPosition(cc.p(140, 75/2))
		...
		headBG:setTag(5)
		headBG:setPosition(cc.p(17,75/2))
