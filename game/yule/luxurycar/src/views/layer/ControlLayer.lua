...
local Game_CMD = appdf.req(appdf.GAME_SRC.."yule.luxurycar.src.models.CMD_Game")

local ControlLayer = class("ControlLayer", cc.Layer)
ControlLayer.BT_BINLI = 11
ControlLayer.BT_BAOMA = 12
ControlLayer.BT_BENCHI = 13
ControlLayer.BT_BAOSHIJIE = 14
ControlLayer.BT_FALALI = 15
ControlLayer.BT_LUHU = 16
ControlLayer.BT_MAIBAHE = 17
ControlLayer.BT_BUJIADI = 18
...
function ControlLayer:ctor( viewParent )
	...
	--加载csb资源
	local csbNode = ExternalFun.loadCSB("game_res/control/ControlLayer.csb", self)
	...
	--宾利
	btn = btn_controlArea:getChildByName("Button_binli");
	btn:setTag(ControlLayer.BT_BINLI);
	btn:addTouchEventListener(btnEvent);
	self.m_btnControlArea[1] = btn;
	--宝马
	btn = btn_controlArea:getChildByName("Button_baoma");
	btn:setTag(ControlLayer.BT_BAOMA);
	btn:addTouchEventListener(btnEvent);
	self.m_btnControlArea[2] = btn;
	--奔驰
	btn = btn_controlArea:getChildByName("Button_benchi");
	btn:setTag(ControlLayer.BT_BENCHI);
	btn:addTouchEventListener(btnEvent);
	self.m_btnControlArea[3] = btn;
	--保时捷
	local btn = btn_controlArea:getChildByName("Button_baoshijie");
	btn:setTag(ControlLayer.BT_BAOSHIJIE);
	btn:addTouchEventListener(btnEvent);
	self.m_btnControlArea[4] = btn;
	--法拉利
	btn = btn_controlArea:getChildByName("Button_falali");
	btn:setTag(ControlLayer.BT_FALALI);
	btn:addTouchEventListener(btnEvent);
	self.m_btnControlArea[5] = btn;
	
	--路虎
	btn = btn_controlArea:getChildByName("Button_luhu");
	btn:setTag(ControlLayer.BT_LUHU);
	btn:addTouchEventListener(btnEvent);
	self.m_btnControlArea[6] = btn;
	--迈巴赫
	btn = btn_controlArea:getChildByName("Button_maibahe");
	btn:setTag(ControlLayer.BT_MAIBAHE);
	btn:addTouchEventListener(btnEvent);
	self.m_btnControlArea[7] = btn;
	--布加迪
	btn = btn_controlArea:getChildByName("Button_bujiadi");
	btn:setTag(ControlLayer.BT_BUJIADI);
	btn:addTouchEventListener(btnEvent);
	...
	--密码输入	
	self.edit_UserGameID = ccui.EditBox:create(cc.size(self.UserGameID:getContentSize().width,self.UserGameID:getContentSize().height), ccui.Scale9Sprite:create("game_res/control/text_field_space.png"))
...
function ControlLayer:showSceneTip(tag)
	...
	local str = string.format("game_res/control/scene_tip_%d.png", tag)
...
function ControlLayer:showPlayerAreaBet(index,arealScore,dwGameID,totleBet)
	...
	if nil ~= item then
		...
		local strlTdTotalScore = nil;
		strlTdTotalScore = ExternalFun.formatScoreText(arealScore)
		userScore:setString(strlTdTotalScore)
...
function ControlLayer:runControl()
	...
	cmddata:pushbyte(Game_CMD.C_CA_SET)
	cmddata:pushbyte(self.m_currentArea - 1)
