...
function Fish:initAnim()
   ...
   -- self.bomType=self.m_data.m_fudaifishtype
    ...
    if self.lockType == 2 then
    	...
    	cc.SpriteFrameCache:getInstance():addSpriteFrames("game_res/fish_move1.plist")
   		cc.Director:getInstance():getTextureCache():addImage("game_res/fish_move1.png")
...
--	end
	...
	if (self.m_data.fish_kind >=  Game_CMD.FishType.FishType_JianYu and self.m_data.fish_kind <=  Game_CMD.FishType.FishType_LiKui) or self.m_data.fish_kind ==  Game_CMD.FishType.FishType_BaoZhaFeiBiao then
		...
		for i=1,nBomb do
			cc.SpriteFrameCache:getInstance():addSpriteFrames("game_res/bomb.plist")
    		cc.Director:getInstance():getTextureCache():addImage("game_res/bomb.png")
