...
local GameViewLayer = appdf.req(appdf.GAME_SRC.."yule.redninebattle.src.views.layer.GameViewLayer")
local GameFrame = appdf.req(appdf.GAME_SRC.."yule.redninebattle.src.models.GameFrame")
...
function GameLayer:SendPlaceJetton(jettonScore, jettonArea)
	...
	cmddata:pushdouble(jettonScore)
...
function GameLayer:onEventGameMessage(sub, dataBuffer)
	...
	elseif sub == Game_CMD.SUB_S_SEND_CARDRECORD then
		self:onGameMessageSendCardRecord(dataBuffer)
	...
	elseif sub ==  Game_CMD.SUB_S_AMDIN_COMMAND then
		self:nGameMessageAdmincontorl(dataBuffer)
	elseif sub == Game_CMD.SUB_S_PEIZHI_USER then
		self:nGameMessageAdminaddpeizhi(dataBuffer)
	elseif sub == Game_CMD.SUB_S_DelPeizhi then
		self:nGameMessageAdminDelpeizhi(dataBuffer)
	elseif sub == Game_CMD.SUB_S_UPALLLOSEWIN then
...
function GameLayer:onGameMessageSendRecord(dataBuffer)
	...
	for i=1,recordcount do
		...
		rec.bWinShunMen = dataBuffer:readbool()
		rec.bWinDuiMen = dataBuffer:readbool()
		rec.bWinDaoMen = dataBuffer:readbool()
...
--开牌记录
function GameLayer:onGameMessageSendCardRecord(dataBuffer)
	local len = dataBuffer:getlen()
	local recordcount = math.floor(len/Game_CMD.CARDRECORD_LEN)
	self._dataModle:clearCardRecord()
	
	print("开牌记录数目", recordcount)
	for i=1,recordcount do
		if nil == dataBuffer then
			break
		end
		local rec = Game_CMD.getEmptyCardRecord()
		rec.m_card1 = dataBuffer:readbyte()
		rec.m_card2 = dataBuffer:readbyte()
		rec.m_card3 = dataBuffer:readbyte()
		rec.m_card4 = dataBuffer:readbyte()
		rec.m_card5 = dataBuffer:readbyte()
		rec.m_card6 = dataBuffer:readbyte()
		rec.m_card7 = dataBuffer:readbyte()
		rec.m_card8 = dataBuffer:readbyte()
		self._dataModle:addCardRecord(rec)
	end

	self._gameView:refreshCardRecord()
end
...
function GameLayer:nGameMessageAdminaddpeizhi(dataBuffer)
	local cmd_table = ExternalFun.read_netdata(Game_CMD.CMD_S_peizhiVec, dataBuffer)
	self._gameView:OnAdmincontorlpeizhi(cmd_table)
end
function GameLayer:nGameMessageAdmincontorl(dataBuffer)
	local cmd_table = ExternalFun.read_netdata(Game_CMD.CMD_S_CommandResult, dataBuffer)
	self._gameView:OnAdmincontorl(cmd_table)
end
function GameLayer:nGameMessageAdminDelpeizhi(dataBuffer)
	local cmd_table = ExternalFun.read_netdata(Game_CMD.CMD_S_DelPeizhi, dataBuffer)
	self._gameView:OnDelPeizhi(cmd_table)
end
function GameLayer:nGameMessageAdminUpalllosewin(dataBuffer)
	local cmd_table = ExternalFun.read_netdata(Game_CMD.CMD_S_UpAlllosewin, dataBuffer)
	self._gameView:OnUpAlllosewin(cmd_table)
end
function GameLayer:executecontrol(cmddata)
	 self:SendData(Game_CMD.SUB_C_AMDIN_COMMAND, cmddata)
end
function GameLayer:ControlAddPeizhi(cmddata)
	 self:SendData(Game_CMD.SUB_C_PEIZHI_USER, cmddata)
end
function GameLayer:ControlDelPeizhi(cmddata)
	 self:SendData(Game_CMD.SUB_C_DelPeizhi, cmddata)
end
function GameLayer:androidxiazhuang(cmddata)
	
	local dataBuffer = CCmd_Data:create(0)
    dataBuffer:setcmdinfo(yl.MDM_GF_GAME, Game_CMD.SUB_C_ANDROIDXIAZHUANG)
