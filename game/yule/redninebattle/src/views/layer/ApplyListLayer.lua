...
local ApplyListItem = appdf.req(appdf.GAME_SRC.."yule.redninebattle.src.views.layer.ApplyListItem")
...
function ApplyListLayer:ctor( viewParent)
	...
	btn:setTag(ApplyListLayer.BT_APPLY)
...
function ApplyListLayer:showLayer()
...
function ApplyListLayer:tableCellAtIndex( view, idx )
	...
	if nil == cell then
		...
		item = ApplyListItem:create()
		item:setPosition(view:getViewSize().width * 0.5+8, 0)
