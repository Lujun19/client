...
local BrnnCMD = appdf.req(appdf.GAME_SRC.."yule.redninebattle.src.models.CMD_Game")
local GameFrame = class("GameFrame")

local RecordMaxNum = 7  --最大路单记录
local CardRecordMaxNum = 3  --最大开牌记录
...
---开牌数据
function GameFrame:addCardRecord(rec)
    table.insert(self.m_vecCardRecord,1, rec)
    local listnum = #self.m_vecCardRecord
    if listnum > CardRecordMaxNum then
        local movenum = listnum - CardRecordMaxNum
        for i=1, movenum do
            table.remove(self.m_vecCardRecord, 1)
        end
    end
end

--获取开牌记录
function GameFrame:getCardRecord(  )
    return self.m_vecCardRecord
end

--清除开牌记录
function GameFrame:clearCardRecord()
    self.m_vecCardRecord = {}
end
------
return GameFrame