...
function GameLogic.SortCardList(cbCardData)
	...
	if not cbCardData or  #cbCardData <= 1  then
		return
	end
	...
	while cbCardData[cbCardCount]== GameLogic.MAGIC_DATA do
		table.remove(cbCardData,cbCardCount)
		table.insert(cbCardData,1,GameLogic.MAGIC_DATA)	
	end

end

--排序
function GameLogic.SortCardListByViewId(cbCardData,viewid)
	--校验
	--assert(type(cbCardData) == "table" and #cbCardData > 0)
	if not cbCardData or  #cbCardData <= 1  then
		return
