...
function GameLayer:onGameSceneFree(dataBuffer)
	...
	for i = 1,6 do
        if nil ~= self._gameView.m_tabSitDownUser[i] then
            self._gameView.m_tabSitDownUser[i]:removeFromParent()
            self._gameView.m_tabSitDownUser[i] = nil
        end
	end
    self._gameView.m_nSelfSitIdx = nil
...
function GameLayer:onEventUserEnter(wTableID, wChairID, useritem)
	...
	self._gameView:setuserAreaBet(useritem.dwGameID,0,0,0,0,0,useritem.bAndroid)
end
