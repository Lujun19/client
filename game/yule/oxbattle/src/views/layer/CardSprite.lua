...
function CardSprite:createCard( cbCardData )
	...
	if nil ~= sp and nil ~= tex and sp:initWithTexture(tex, tex:getContentSize()) then
		...
		sp.m_cardValue = GameLogic.GetCardValue(cbCardData) --math.mod(cbCardData, 16)--bit.band(cbCardData, 0x0F)
		sp.m_cardColor = GameLogic.GetCardColor(cbCardData) --math.floor(cbCardData / 16)--bit.rshift(bit.band(cbCardData, 0xF0), 4)
...
function CardSprite:setCardValue( cbCardData )
	...
	self.m_cardValue = GameLogic.GetCardValue(cbCardData) --math.mod(cbCardData, 16) --bit.band(cbCardData, 0x0F)
	self.m_cardColor = GameLogic.GetCardColor(cbCardData) --math.floor(cbCardData / 16) --bit.rshift(bit.band(cbCardData, 0xF0), 4)
