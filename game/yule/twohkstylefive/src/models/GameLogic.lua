...
GameLogic.MAX_CARDCOUNT			= 5--最大牌数
GameLogic.MASK_VALUE			= 0X0F--牌值掩码
...
function GameLogic:GetCardType( cbCardDataTable)
	...
	if cbCardCount == 5 then
		...
		for i=2,cbCardCount  do
            ...
            if cbLineCard == false and cbFirstValue == 8 then
                local nCount = 0
                for i=2,cbCardCount - 1  do
                    local tempValueEx = self:GetCardValue(cbCardDataTable[i])
                    if (cbFirstValue + i - 1) == tempValueEx then 
                        nCount = nCount + 1
                    end
                end
                local tempValueA = self:GetCardValue(cbCardDataTable[cbCardCount])
                if nCount == 3 and tempValueA == 1 then
                    cbLineCard = true
                end
	...
	repeat
		...
		for i=1,cbLast do
			if (cbSortValue[i] > cbSortValue[i+1])
				...
				bSorted = false;--设置标志
				cbCardDataTable[i], cbCardDataTable[i + 1] = cbCardDataTable[i + 1], cbCardDataTable[i];--扑克数据
				cbSortValue[i], cbSortValue[i + 1] = cbSortValue[i + 1], cbSortValue[i];--排序权位
			end
		end
		cbLast = cbLast - 1;
	until bSorted ~= false;
