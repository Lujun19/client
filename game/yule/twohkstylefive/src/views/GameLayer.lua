...
local GameLogic     = appdf.req(appdf.GAME_SRC.."yule.twohkstylefive.src.models.GameLogic")
local GameViewLayer = appdf.req(appdf.GAME_SRC.."yule.twohkstylefive.src.views.layer.GameViewLayer")
...
function GameLayer:OnInitGameEngine()
    ...
    self.LookCardUser={false,false,false,false,false}      --一已看牌用户
end
-- 重置游戏数据
...
function GameLayer:onEventGameMessage(sub,dataBuffer)
    ...
    elseif sub == cmd.SUB_S_LOOKCARD then 
...
function GameLayer:onSubGameStart(dataBuffer)
    ...
    if self._gameFrame.bEnterAntiCheatRoom == true then--防作弊判断
        self:dismissPopWait()
    end
    ...
    self._gameView:SetCellScore(self.m_lCellScore)       --底分
    self._gameView:SetMaxCellScore(self.m_lDrawMaxScore)
    self:updateOperatorText(self.m_wCurrentUser)         --更新底部提示文字
    --初始化已有玩家数据
    for i = 1, cmd.GAME_PLAYER do
    ...
    self.LookCardUser={false,false,false,false,false} 
    ExternalFun.playSoundEffect("GAME_START.wav")--音效
end
--用户加注
...
function GameLayer:onSubLookCard(dataBuffer)
    local cmd_table = ExternalFun.read_netdata(cmd.CMD_S_LookCard, dataBuffer);
    local lookCard=cmd_table.bLook
    for i=1,cmd.GAME_PLAYER do
        if lookCard[1][i]==true then -- and self.LookCardUser[i]==false 
            local viewid = self:SwitchViewChairID(i-1)
            if viewid==cmd.MY_VIEWID and self._gameView.isMeLookCard==true then
                self._gameView:showLookCard(viewid)
            elseif viewid~=cmd.MY_VIEWID and self._gameView.isMeLookCard~=true then
                self._gameView:showLookCard(viewid)
            end
        end
    end
    self._gameView.isMeLookCard=false

--    for j=1,cmd.GAME_PLAYER do
--        self.LookCardUser[j]=lookCard[1][j]
--    end
end
--发牌信息
...
function GameLayer:onFollowScore()
    ...
    --判断是否在可范围内
    local totalScore  = self.m_lTotalScore[MyChair] + addScore
...
function GameLayer:onAddScore(index)
    ...
    self._gameView:showTip(viewid,3)
    --下注金额
    local addScore = cmd.ZHU_MA[not index and 1 or index] + self.m_llLastScore
...
function GameLayer:onAddScoreButton()
    ...
    self._gameView.btnAddGold:setVisible(true)
