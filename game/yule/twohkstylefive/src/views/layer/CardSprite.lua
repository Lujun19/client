...
function CardSprite:createCard( cbCardData )
	...
	sp.m_strCardFile = "game/yule/twohkstylefive/res/cards_s.png"
	...
	if nil ~= sp and nil ~= tex and sp:initWithTexture(tex, tex:getContentSize()) then
		...
		sp.m_cardValue = math.mod(cbCardData, 16)
		sp.m_cardColor = math.floor(cbCardData / 16)
		sp:updateSprite()--扑克背面
		sp:createBack()
		return sp
	end
	return nil;
...
function CardSprite:setCardValue( cbCardData )
	...
	self.m_cardValue = math.mod(cbCardData, 16) 
	self.m_cardColor = math.floor(cbCardData / 16) 
	self:updateSprite();
end
--更新纹理资源
function CardSprite:updateSprite(  )
	...
	if 0 ~= m_cardData then
		...
		rect = cc.rect(2 * CARD_WIDTH, 4 * CARD_HEIGHT, CARD_WIDTH, CARD_HEIGHT)--使用背面纹理区域
	end
...
function CardSprite:createBack(  )
	...
	local rect     = cc.rect(2 * CARD_WIDTH, 4 * CARD_HEIGHT, CARD_WIDTH, CARD_HEIGHT)--纹理区域
