...
function PriGameLayer:onButtonClickedEvent( tag, sender )
    ...
    elseif BTN_INVITE == tag then
        ...
        PriRoom:getInstance():getPlazaScene():popTargetShare(function(target, bMyFriend)
            ...
            local shareTxt = "诈金花约战 房间ID:" .. self.m_atlasRoomID:getString() .. " 局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit
            shareTxt = shareTxt .. " 诈金花游戏精彩刺激, 一起来玩吧! "
            local friendC = "诈金花约战 房间ID:" .. self.m_atlasRoomID:getString() .. " 局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit
            local url = GlobalUserItem.szSpreaderURL or BaseConfig.WEB_HTTP_URL
            if bMyFriend then
            ...
            elseif nil ~= target then
                GlobalUserItem.bAutoConnect = false
                MultiPlatform:getInstance():shareToTarget(target, sharecall, "诈金花约战", shareTxt, url, "")
            end
        end)
    elseif BTN_SHARE == tag then
        ...
        PriRoom:getInstance():getPlazaScene():popTargetShare(function(target, bMyFriend)
            ...
            captureScreenWithArea(area, "grade_share.png", function(ok, savepath)
...
function PriGameLayer:onRefreshInviteBtn()
    if self._gameLayer.m_cbGameStatus ~= 0 or 0 ~=  PriRoom:getInstance().m_tabPriData.dwPlayCount then --空闲场景
    ...
    for i = 1, chairCount do
        ...
        if nil ~= cellbg then
            ...
            if nil ~= useritem then
                print("结算时获取到用户信息")
                -- 头像
                local head = PopupInfoHead:createNormal(useritem, 115)
