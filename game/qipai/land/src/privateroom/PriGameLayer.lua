...
function PriGameLayer:onButtonClickedEvent( tag, sender )
    ...
    elseif BTN_INVITE == tag then
        PriRoom:getInstance():getPlazaScene():popTargetShare(function(target, bMyFriend)
            ...
            local shareTxt = "局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit .. "局".. " 底分:".. PriRoom:getInstance().m_tabPriData.lCellScore
            local friendC = "斗地主房间ID:" .. self.m_atlasRoomID:getString() .. " 局数:" .. PriRoom:getInstance().m_tabPriData.dwDrawCountLimit.. " 底分:".. PriRoom:getInstance().m_tabPriData.lCellScore
            local url = GlobalUserItem.szWXSpreaderURL or yl.Http_DownUrl
            if bMyFriend then
            ...
            elseif nil ~= target then
                ...
                MultiPlatform:getInstance():shareToTarget(
                    target, 
                    sharecall, 
                    "【风龙约战房间: " .. self.m_atlasRoomID:getString() .. " 】", 
                    shareTxt .. " 斗地主游戏精彩刺激, 一起来玩吧! ", 
                    url, 
                    "")
            end
        end)
