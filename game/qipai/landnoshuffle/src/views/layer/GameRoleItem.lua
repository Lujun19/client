...
local popPosition = {}
popPosition[cmd.LEFT_VIEWID] = cc.p(109, 397)
popPosition[cmd.RIGHT_VIEWID] = cc.p(812, 387)
popPosition[cmd.MY_VIEWID] = cc.p(114, 100)
local popAnchor = {} 
popAnchor[cmd.LEFT_VIEWID] = cc.p(0, 1)
popAnchor[cmd.RIGHT_VIEWID] = cc.p(1, 1)
...
--[[    -- 用户头像
    ...
    csbNode:addChild(head)--]]
		    ...
		    -- 用户头像
    local head = PopupInfoHead:createNormalCircle(userItem, 87,("Circleframe.png"))
	head:setPosition(-8, 0)
...
function GameRoleItem:textChat( str )
    ...
    if nil == self.m_labChat then
        self.m_labChat = cc.LabelTTF:create(str, "base/fonts/round_body.ttf", 20, cc.size(150,0), cc.TEXT_ALIGNMENT_CENTER)       
		self.m_labChat:setFontFillColor(cc.c3b(38,15,0))   
