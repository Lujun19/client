...
function GameLogic:GetCardIndex(nCardData)
    ...
    local nCardValue = self:GetCardValue(nCardData)
    local nCardColor = self:GetCardColor(nCardData)
    nCardColor = bit.rshift(nCardColor, 4)
...
function GameLogic:GetCardType(cbCardData, cbCardCount)
    ...
    elseif cbCardCount == 1 then    --单牌
        return GameLogic.CT_SINGLE, cbCardData[1]
    ...
    if tagAnalyseResult[1][3] > 0 then
		...
		-- 一个三张的
		local order = "positive"
        if tagAnalyseResult[1][3]*3 == cbCardCount then
           return GameLogic.CT_THREE_LINE, order
        end
		local threeValue = GameLogic:GetCardLogicValue(tagAnalyseResult[2][3][1])
        if tagAnalyseResult[1][3]*4 == cbCardCount then
			local oneValue = GameLogic:GetCardLogicValue(tagAnalyseResult[2][1][1] or tagAnalyseResult[2][2][1])
			if	oneValue > threeValue then
				order = "reverse"
			end
           return GameLogic.CT_THREE_TAKE_ONE, order
        end
        if tagAnalyseResult[1][3]*5 == cbCardCount  and tagAnalyseResult[1][2] == tagAnalyseResult[1][3] then
			local twoValue = GameLogic:GetCardLogicValue(tagAnalyseResult[2][2][1])
			if	twoValue > threeValue then
				order = "reverse"
			end
           return GameLogic.CT_THREE_TAKE_TWO, order
        end
        return GameLogic.CT_ERROR
    end
    --两张判断
...
--出牌搜索
function GameLogic:SearchOutCardTouch(cbHandCardData,cbHandCardCount,cbTurnCardData,cbTurnCardCount) 
    --结果数目
    local cbResultCount = 1
    --扑克数目
    local cbResultCardCount = {}
...
function GameLogic:AnalysebDistributing(cbCardData, cbCardCount)
    ...
    for i=1,cbCardCount do
        if cbCardData[i]~=0 then
            ...
            local color = bit.rshift(cbCardColor,4) + 1
...
function GameLogic:MakeCardData(cbValueIndex,cbColorIndex)
    --print("构造扑克 " ..bit.bor(bit.lshift(cbColorIndex,4),cbValueIndex)..",".. GameLogic:GetCardLogicValue(bit.bor(bit.lshift(cbColorIndex,4),cbValueIndex)))
    return bit.bor(bit.lshift(cbColorIndex,4),cbValueIndex)
end
...
function GameLogic:DeepCopyArray(source, target)
	for k=1, #source do
		target[k] = source[k]
	end
end
