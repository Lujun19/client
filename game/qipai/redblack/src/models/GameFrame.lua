        ...
        repeat
            ...
            if true == bFound then
                ...
                elseif 6 == bill.m_cbIndex then
                    ...
                    if (bill.m_pRecords[bill.m_cbIndexWithoutPing] == rec.m_cbGameResult) then
                        --大于六行，新加一列
...
function GameFrame.calcuteJetton( llScore, bAllJetton )
    ...
    if bAllJetton then
		...
		--计算1000下注数量
        local nCount = math.floor(tmpScore / 1000);
        if (nCount > 0) then        
        ...
        if (nCount > 0) then
            ...
            tmpScore = tmpScore - nCount * 50;
        end
        
        --计算10下注数量
        nCount = math.floor(tmpScore / 10);
        ...
        if (nCount > 0) then
            ...
            tmpScore = tmpScore - nCount * 10;
        end
        
        --计算5下注数量
        nCount = math.floor(tmpScore / 5);
        if (nCount > 0) then
            local info = {m_cbIdx = 0, m_cbCount = 0, m_llScore = 0};
            info.m_llScore = 5;
            info.m_cbCount = nCount;
            ...
            tmpScore = tmpScore - nCount * 5;
        end
        
        --计算1下注数量
        nCount = math.floor(tmpScore / 1);
        if (nCount > 0) then
            local info = {m_cbIdx = 0, m_cbCount = 0, m_llScore = 0};
            info.m_llScore = 1;
            info.m_cbCount = nCount;
