...
local Game_CMD = appdf.req(appdf.GAME_SRC.."qipai.redblack.src.models.CMD_Game")
...
function ControlLayer:ctor( viewParent )
	...
	for i=1,5 do
		...
		self.m_btnControlTimes[i]:setTag(i+9)
	...
	for i=1,3 do
		...
		self.m_areaTotalText[i]:setString("")	
	end
...
--各区域总下注
function ControlLayer:setAreaTotalBet(index,totalScore)
	
	self.m_areaTotalScore[index] = (self.m_areaTotalScore[index] or 0)+ totalScore
	local str = string.format("%.2f",self.m_areaTotalScore[index])
	self.m_areaTotalText[index]:setString(str)
...
function ControlLayer:runControl()
	...
	-- 可以单独选择谁赢或者什么牌型
	if not (count == 1 or (count == 2 and (self.m_bWinArea[1] == 1 or self.m_bWinArea[2] == 1))) then
