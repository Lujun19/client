...
--[[	local m_tableView = cc.TableView:create(content:getContentSize())
	...
	self.m_playerNumText:setString("当前共有0位玩家")
	--翻页按钮
...
--	self.m_tableView:reloadData()
	...
	self.m_playerNumText:setString("当前共有" .. num .. "位玩家")
	
	self.m_totlePage = math.ceil(num/15)
	...
	for i=1, 15 do
		...
		if item ~= nil then
			...
			if nil ~= item.dwGameID then
				szNick = item.dwGameID
			end
			nickName:setString("ID: "..szNick)
