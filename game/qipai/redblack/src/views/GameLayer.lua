...
function GameLayer:onEventGameSceneFree( dataBuffer )
	...
	self._gameView:updateLbottomPourImpose(cmd_table.lBottomPourImpose)
   ...
   self._gameView:onGetSitDownInfo(cmd_table.occupyseatConfig, cmd_table.wOccupySeatChairID[1])
	...
	--设置退出按钮是否可用
	self._gameView:setExitEnabled(true)
--	self._gameView:updateTopCard(true)
	self._gameView:setRepeat(false)
	self._gameView:clearEndAnimation()
end
...
function GameLayer:onEventGameSceneJetton( dataBuffer )
    ...
    local lScore = 0;
    local ll = 0;
	--self._gameView.m_betAreaLayout:removeAllChildren()
	...
	--设置退出按钮是否可用
	self._gameView:setExitEnabled(false)
	--闪烁停止
	self._gameView:jettonAreaBlinkClean()
	self._gameView:updateTopCard(true)
	self._gameView:setRepeat(false)
	self._gameView:clearEndAnimation()
end
...
function GameLayer:onEventGameSceneEnd( dataBuffer )
	...
	if self._gameView.m_lHaveJetton > 0 then
		bJoin = true
	end
	...
	tabRes.m_cardsType = cmd_table.cbCardType[1]
    self._gameView:onGetGameCard(tabRes, false, cmd_table.cbTimeLeave,cmd_table.cbWinArea[1], cmd_table.cbTimeLeave)
...
function GameLayer:onSubGameFree( dataBuffer )
	...
	--控制结束
	if self.cmd_gamefree.cbControl == 0 then
		if self._gameView.m_controlLayer ~=nil then
...
function GameLayer:onSubGameStart( dataBuffer )
	...
	self._gameView:updateLbottomPourImpose(self.cmd_gamestart.lBottomPourImpose)
...
function GameLayer:onSubGameEnd( dataBuffer )
    ...
    tabRes.m_cardsType = cmd_table.cbCardType[1]
    self._gameView:onGetGameCard(tabRes, true, cmd_table.cbTimeLeave,cmd_table.cbWinArea[1])
	
	self._gameView:onGetGameRecord(tabRes,cmd_table.cbWinArea[1])
...
--[[		serverrecord.bPlayerTianGang = dataBuffer:readbool();
		serverrecord.bBankerTianGang = dataBuffer:readbool();
		...
		serverrecord.cbWinType = dataBuffer:readbyte();
		serverrecord.cbGameResult = dataBuffer:readbyte();
...
--[[        if serverrecord.cbPlayerCount > serverrecord.cbBankerCount then
            rec.m_cbGameResult = g_var(cmd).AREA_ZHONG;
        elseif serverrecord.cbPlayerCount < serverrecord.cbBankerCount then
            rec.m_cbGameResult = g_var(cmd).AREA_BAI;
        elseif serverrecord.cbPlayerCount == serverrecord.cbBankerCount then
...
--	self._dataModle:getRecords()
    --self._gameView:updateWallBill()
	self._gameView:updateGameRecord()
end
...
--[[	--刷新座位玩家
	...
	self._gameView:setPlayerEnter(useritem.dwGameID,useritem.bAndroid)
end
...
function GameLayer:onEventUserStatus(useritem,newstatus,oldstatus)
   -- print("change user " .. useritem.wChairID .. "; nick " .. useritem.szNickName)
    if newstatus.cbUserStatus == yl.US_FREE then
		...
		self._gameView:removeuserAreaBet(useritem.dwGameID)
    else
        --刷新用户信息
