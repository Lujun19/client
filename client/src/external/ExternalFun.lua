...
function ExternalFun.read_datahelper2( param, debug_mode2 )
	if debug_mode2 then
		print("read: " .. param.strkey .. " helper");
		LogAsset:getInstance():logData("read: " .. param.strkey .. " helper", true)
	end
	...
	if nil ~= param.lentable then
		...
		if debug_mode2 then
			print("depth:" .. depth);
			LogAsset:getInstance():logData("depth:" .. depth, true)
		end
		...
		for i=1,depth do
			...
			if debug_mode2 then
				print("entryLen:" .. entryLen);
				LogAsset:getInstance():logData("entryLen:" .. entryLen, true)
			end
			...
			for i=1,entryLen do
				...
				if debug_mode2 then
					if type(entry) == "boolean" then
						print("value:".. (entry and "true" or "false"))
						LogAsset:getInstance():logData("value:".. (entry and "true" or "false"), true)
					else
						print("value:" .. entry);
						LogAsset:getInstance():logData("value:" .. entry, true)
					end
				end
		...
		if debug_mode2 then
			...
			if type(value) == "boolean" then
				print("value:".. (value and "true" or "false"))
				LogAsset:getInstance():logData("value:".. (value and "true" or "false"), true)
			else
				print("value:" .. value);
				LogAsset:getInstance():logData("value:" .. value, true)
			end
			return value;
		else
			return param.fun();
...
function ExternalFun.read_netdata( keyTable, dataBuffer )
	...
	for k,v in pairs(keyTable) do
		...
		LogAsset:getInstance():logData("keys2", true)
		LogAsset:getInstance():logData(keys["t"], true)
		...
		-- LogAsset:getInstance():logData("key=="..key, true)
    -- LogAsset:getInstance():logData(cjson.encode(cmd_data), true)
		if nil ~= keyFun then
...
function ExternalFun.read_netdata2( keyTable, dataBuffer )
	local cmd_table = {};
	...
	for k,v in pairs(keyTable) do
		...
		LogAsset:getInstance():logData("keys", true)
		LogAsset:getInstance():logData(keys["t"], true)
		...
		if "byte" == keyType then
			keyFun = function() return dataBuffer:readbyte(); end
		elseif "int" == keyType then
			LogAsset:getInstance():logData("int", true)
			keyFun = function() return dataBuffer:readint(); end
		elseif "word" == keyType then
			keyFun = function() return  dataBuffer:readword(); end
		elseif "dword" == keyType then
		...
		if nil ~= keyFun then
			cmd_table[key] = ExternalFun.read_datahelper2({strkey = key, lentable = lenT, fun = keyFun},true);
