...
function PopupInfoLayer:onNearUserInfo( event )
	...
	if nil ~= nearuser then
		...
		self.m_textIp:setString(self.m_userInfo.szAdressLocation)
...
function PopupInfoLayer:refresh( useritem, popPos ,anr)
	...
	if useritem.dwGameID == GlobalUserItem.dwGameID then
		...
		ipLocation = GlobalUserItem.szIpLocation
	else 
...
--[[	if (userItem.szChangeLogonIP and userItem.szChangeLogonIP ~= "") then
	...
	--useritem.szAdressLocation
	self.m_textIp:setString(useritem.szAdressLocation)
