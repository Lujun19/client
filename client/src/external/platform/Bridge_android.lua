...
function Bridge_android.setAlias( account )
    local args = { account }
	local sigs = "(Ljava/lang/String;)V" 
    local ok,ret = luaj.callStaticMethod(BRIDGE_CLASS,"setAlias",args, sigs)
    if not ok then
        local msg = "luaj error:" .. ret
        print(msg)  
        return false, msg   
...
function Bridge_android.deleteAlias()
    local args = {}
	local sigs = "()V" 
    local ok,ret = luaj.callStaticMethod(BRIDGE_CLASS,"deleteAlias",args, sigs)
    if not ok then
        local msg = "luaj error:" .. ret
        print(msg)  
        return false, msg   
...
function Bridge_android.presentServiceViewController( userid,signature,time,url, headurl)
    local args = 
    { 
                    userid,
                    signature,
	...
	--public static void gotoFeedBack( int userid,String signature)
    local ok,ret = luaj.callStaticMethod(BRIDGE_CLASS,"gotoFeedBack",args, sigs)
    if not ok then
        local msg = "luaj error:" .. ret
        print(msg)  
        return 0, msg   
...
function Bridge_android.openWebview( url)
	...
	local sigs = "(Ljava/lang/String;)V" 
	--public static void gotoFeedBack( int userid,String signature)
    local ok,ret = luaj.callStaticMethod(BRIDGE_CLASS,"gotoWeb",args, sigs)
    if not ok then
        local msg = "luaj error:" .. ret
        print(msg)  
        return 0, msg   
...
--设置极光推送别名,传入参数 ：userid ，用于给该“userid”的用户 单独推送 远程通知消息 
function Bridge_android.openQQ()
    local args = {}
	local sigs = "()V" 
    local ok,ret = luaj.callStaticMethod(BRIDGE_CLASS,"openQQ",args, sigs)
    if not ok then
        local msg = "luaj error:" .. ret
        print(msg)  
        return false, msg   
