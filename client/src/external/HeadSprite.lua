...
local SYS_HEADSIZE = 245
local SYS_HEADCOUNT = 4
...
--创建普通玩家头像
function HeadSprite:createNormalCircle( useritem ,headSize,Circleframe)
	if nil == useritem then
		--return
	end
	useritem = HeadSprite.checkData(useritem)
	...
	if nil ~= clipSp then
		...
		local selfSize = sp:getContentSize()
		--local scaleX = headSize / selfSize.width
		--local scaleY = headSize / selfSize.height
		
		--local selfBoudingBox = sp:getBoundingBox()
...
--创建裁剪玩家头像
function HeadSprite:createClipHeadCircle( useritem, headSize, clippingfile )
	if nil == useritem then
		--return
	end
	useritem = HeadSprite.checkData(useritem)
	...
	if nil ~= clipSp then
		...
		local selfSize = sp:getContentSize()
		clip:setPosition(cc.p(selfSize.width * 0.50-2, selfSize.height * 0.50-10))
		sp:addChild(clip)
		return sp
	end
...
function HeadSprite:reSize(reSize)
	-- body
...
function HeadSprite:updateHead( useritem,newSize)
	if nil == useritem then
		return
	end
	...
	if self.m_bFrameEnable and false == bAntiCheat then
		...
		local scaleRate = 3.4--self.m_tabFrameParam._scaleRate or deScale
		local zorder =0.5-- self.m_tabFrameParam._zorder or 1
	...
	local str = ""
	if (useritem.cbGender == yl.GENDER_MANKIND) then
		if (useritem.wFaceID > 0) then
			local index = (useritem.wFaceID - 1) % SYS_HEADCOUNT
			if (index < 0) then
				index = 0
			end		
			str = string.format("man_%d.png", index)
		else
			str = string.format("man_%d.png", 0)
		end
		str = "public/man_0.png"
	else
		if (useritem.wFaceID > 0) then
			local index = (useritem.wFaceID - 1) % SYS_HEADCOUNT
			if (index < 0) then
			...
			str = string.format("women_%d.png", index)
		else
			str = string.format("women_%d.png", 0)
		end
		str = "public/women_0.png"
	end
	...
	if newSize == nil then 
		...
		self:setContentSize(cc.size(newSize,newSize))
	end
...
function HeadSprite:enableHeadFrame( bEnable, frameparam )
	...
	local scaleRate =3.4-- frameparam._scaleRate or deScale
	local zorder =0.5--frameparam._zorder or 1
	self:updateHeadFrame(framefile, scaleRate):setLocalZOrder(zorder)
    end
end
...
function HeadSprite:updateHeadFrame(framefile, scaleRate)
	...
	--if nil == self.m_spFrame then
		...
		local positionRate =cc.p(0, 0)-- self.m_tabFrameParam._posPer or cc.p(0.5, 0.5)
        self.m_spFrame:setAnchorPoint(ccp(0.5, 0.5)) 
        local vipIdx = self.m_useritem.cbMemberOrder or 0
        if vipIdx==6 then
		self.m_spFrame:setPosition(128, 140)
        
        elseif vipIdx ==5 then
        self.m_spFrame:setPosition(110, 130)
        elseif vipIdx == 4 then
        self.m_spFrame:setPosition(110, 120)
        elseif vipIdx == 3 then
        self.m_spFrame:setPosition(110, 120)
        elseif vipIdx == 2 then
        self.m_spFrame:setPosition(110, 120)
        elseif vipIdx == 1 then
        self.m_spFrame:setPosition(105, 120)
        end
...
function HeadSprite:initHeadSprite( useritem )
	...
	elseif nil ~= useritem.dwCustomID and 0 ~= useritem.dwCustomID then
		...
		if bHave then
			...
			--网络请求
			local url = BaseConfig.WEB_HTTP_URL .. "/WS/UserFace.ashx?customid=" .. useritem.dwCustomID
			self:downloadFace(url, path, filename, function(downloadfile)
				--判断是否是自己头像
	        	local selffile = filename
	        	if selffile == downloadfile then
