...
function WelcomeScene:askServerList(  )
	-- body

	local url = "http://xiaosheng6.qd1.itbaohe.cn/server/domain.asmx/HelloWorld"
	local vcallback= function (data)
		-- body
		if type(data) == "table" and #data ~= 0 then 
		    ...
		    -- local info = {}
		    -- info[1] ={}
		    -- info[1]["domain"] ="116.31.99.118"
		    -- info[1]["port"] = "8610"
		    -- info[2] ={}
		    -- info[2]["domain"] ="116.31.99.118"
		    -- info[2]["port"] = "8600"
		  --  yl.newServerList = data
		    -- yl.LOGONSERVER = yl.newServerList[1]["domain"]
		    -- yl.LOGONPORT = yl.newServerList[1]["port"]
		end
	
	end
	appdf.onHttpHtmlJsonTable(url,"get", "", vcallback)
end

function WelcomeScene:testIpAddress( ... )
	-- body
	local url = "http://ip.taobao.com/service/getIpInfo.php?ip=myip"
	--local url = "http://myip.dnsomatic.com"
	--local url = "http://www.ip138.com/"
	appdf.onHttpGetIpAdress(url,"get", "", function(ok,ipData)
		-- body
			if ok then 

					local data = ipData.data
					dump(data)
					--GlobalUserItem.szIpLocation = datatable.result.area
					if ipData.code and ipData.code ~= 0 then 
						--GlobalUserItem.szIpLocation = "获取失败"
					else
						local data = string.split(data.ip,".")
						
						local str = string.format("%s %s %s %s", data[1], data[2],data[3],data[4])
						local str2 = string.format("%s.%s.%s.%s", data[1], data[2],data[3],data[4])
						local url = "http://ip.taobao.com/service/getIpInfo.php?ip="
						appdf.onHttpGetIpAdress(url,"get", str2, function(ok,ipData )
							dump(ipData)
							local data = ipData.data
							if ipData.code and  ipData.code == 0 then 
								 local str = data.region..data.city
								dump(str)
								--
								-- if str == "" then 
							...
							-- print("ipDataipDataipDataipData")
							-- dump(ipData)

						end)
					--	dump(str)
						--GlobalUserItem.szIpLocation = string.format("%s %s", data.region, data.city)
					end
			-- 				AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {ctor = {GlobalUserItem.szIpLocation,function(ok)
			
			-- --	os.exit(0)
		
			-- end, false, 2}, canrepeat = false}, VIEW_LIST.QUERY_DIALOG_LAYER)
				else
					print("http://myip.dnsomatic.com")
				--	GlobalUserItem.szIpLocation = "获取失败"
				end
...
function WelcomeScene:checkNewVersion()
	 		    ...
	 		    if succeed == true then
				    ...
				    --游戏列表
 				    local rows = databuffer["gamelist"]
						...
						LogAsset:getInstance():logData(cjson.encode("gamelist"),true)
						LogAsset:getInstance():logData(cjson.encode(rows),true)
	...
	LogAsset:getInstance():logData(cjson.encode("gamelist"),true)
	LogAsset:getInstance():logData(cjson.encode(BaseConfig.WEB_HTTP_URL),true)
...
function WelcomeScene:checkWebUrlInfo(func)
	...
	local function callback(code, url)
		if (code == 0) then
			...
			func(code)
		--	print("没有一个网站可以使用，网站全挂")
			AppFacade:getInstance():sendNotification(GAME_COMMAMD.POP_VIEW, {Name = VIEW_LIST.QUERY_DIALOG_LAYER})
			AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {ctor = {"请检查您的网络，稍后重试",function(ok)
