...
local DuiHuanMediator = class("DuiHuanMediator", BaseMediator)
local logincmd = appdf.req(appdf.HEADER_SRC .. "CMD_LogonServer")

function DuiHuanMediator:listNotificationInterests()
...
function DuiHuanMediator:handleNotification(notification)
...
function DuiHuanMediator:onRegister()
...
function DuiHuanMediator:onExchangeBankScore(e, score, exchangeType)
...
--function DuiHuanMediator:onBindBankInfo(e, realName, bankNo, bankName, bankAddr, alipayID)
function DuiHuanMediator:onBindBankInfo(e, BankInfo)
...
function DuiHuanMediator:onSubOperateSuccess(pData)
	local lResultCode = pData:readint()
	local szDescribe = pData:readstring()
	self.viewComponent:emit("M2V_ShowToast", szDescribe)
end

function DuiHuanMediator:onSubOperateFailure(pData)
	local lResultCode = pData:readint()
	local szDescribe = pData:readstring()
	self.viewComponent:emit("M2V_ShowToast", szDescribe)
end

function DuiHuanMediator:onCheckBindBankInfo(e)
	--先读取缓存数据 没有就请求网络数据
...
function DuiHuanMediator:onQueryBankInfo(e)
	AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {ctor = {"查询银行信息中..."}, canrepeat = false}, VIEW_LIST.POPWAIT_LAYER)	
...
function DuiHuanMediator:onSaveBankScore(e, score)
	if (score == 0) then
...
function DuiHuanMediator:onTakeBankScore(e, insurepass, score)
	if (score == 0) then
...
function DuiHuanMediator:onQueryBankRecord(e)
...
function DuiHuanMediator:onQueryExchangeRecord(e)
...
function DuiHuanMediator:onSubInsureSuccess(pData)
...
function DuiHuanMediator:onSubInsureFailue(pData)
...
--绑定手机
function DuiHuanMediator:onBindPhone(e)

end

return DuiHuanMediator