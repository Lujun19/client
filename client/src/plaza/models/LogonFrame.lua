...
function LogonFrame:onSubLogonEvent(sub,pData)
	...
	--	GlobalUserItem.szIpAdress = MultiPlatform:getInstance():getClientIpAdress() 
		...
		-- AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {ctor = {GlobalUserItem.szIpAdress,function(ok)
			
		-- 	--	os.exit(0)
		
		-- 	end, false, 2}, canrepeat = false}, VIEW_LIST.QUERY_DIALOG_LAYER)
		-- print("获取ip地址")
		-- dump(GlobalUserItem.szIpAdress)
		...
		-- body
				...
				if ok then 
					...
					if data.ip == "" then 
						...
						GlobalUserItem.szIpLocation = string.format("%s %s %s %s", data[1], data[2],data[3],data[4])
					end
...
function LogonFrame:sendVisitor()
	...
	self._szMachine = MultiPlatform:getInstance():getMachineId()
  --  self._szMachine = "A501164B366ECFC9E749163873094D94"
   -- self._szMobilePhone =  self._szMachine
