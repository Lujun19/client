...
--[[	userItem.lScore			= dataBuffer:readscore(int64):getvalue()
	...
	userItem.szAdressLocation = self:Randomget(userItem.dwGameID)

	if (userItem.szChangeLogonIP and userItem.szChangeLogonIP ~= "") then
		--使用伪造的IP归属地
		--userItem.szAdressLocation = userItem.szChangeLogonIP
	else
		-- appdf.onHttpJsionTable(string.format("http://apis.juhe.cn/ip/ip2addr?ip=%s&key=74bb0395667b8d0588df9ebd6fc36a12", userItem.szIpAddress),"get","",function (datatable)
		...
		-- 		local code = datatable.resultcode
		-- 		if (code == "200") then
		-- 		--if (code == "ok") then
		-- 			userItem.szAdressLocation =  datatable.result.area
		...
		-- 		end
		-- 	else--请求失败
		-- 	   -- appdf.onHttpJsionTable(string.format("http://ip.taobao.com/service/getIpInfo.php?ip=%s", userItem.szIpAddress),"get","",function (datatable)
  --              appdf.onHttpJsionTable(string.format("http://apis.juhe.cn/ip/ip2addr?ip=%s&key=74bb0395667b8d0588df9ebd6fc36a12", userItem.szIpAddress),"get","",function (datatable)
		-- 	    --appdf.onHttpJsionTable("http://api.ip138.com/query/?ip="..userItem.szIpAddress.."&datatype=jsonp&token=99f2651ade06917e66d9f977e3c66460","get","",function (datatable)
		-- 	    if type(datatable) == "table" then
		...
		-- 		    --if (code == "ok") then
  --                    userItem.szAdressLocation =  datatable.result.area
		-- 			   -- userItem.szAdressLocation =  string.format("%s%s市",datatable.data.region, datatable.data.city)
		-- 			    --userItem.szAdressLocation = string.format("%s %s %s %s", datatable.data[1], datatable.data[2], datatable.data[3], datatable.data[4])
		-- 		    else
		-- 			    appdf.onHttpJsionTable(string.format("http://apis.juhe.cn/ip/ip2addr?ip=%s&key=74bb0395667b8d0588df9ebd6fc36a12", userItem.szIpAddress),"get","",function (datatable)
		-- 	            --appdf.onHttpJsionTable("http://api.ip138.com/query/?ip="..userItem.szIpAddress.."&datatype=jsonp&token=99f2651ade06917e66d9f977e3c66460","get","",function (datatable)
		...
		-- 		            end
  --                         else--请求失败
		-- 		            userItem.szAdressLocation = self.Randomget()
  --                           --self.Randomget()
		--                 end
  --                      end)
		-- 		    end
...
function GameFrameEngine:Randomget(gameId)
	--local Random=  math.random(1 , 9)
	   local Random  = gameId % 9

        if Random == 1 then
           return "广东省深圳市"           --case 1
        elseif Random ==2 then
          return "福建省厦门市"   --case 2
        elseif Random ==3 then
          return "湖南省怀化市"   --case 3
        elseif Random ==4 then
          return "安徽省黄山市"   --case 4
        elseif Random ==5 then
          return "山东省青岛市"   --case 5
        elseif Random ==6 then
          return "广西省北海市"   --case 6
        elseif Random ==7 then
          return "云南省丽江市"   --case 7
        elseif Random ==8 then
          return "湖南省湘潭市"   --case 8
        elseif Random ==9 then
          return "河北省承德市"   --case 9
        elseif Random ==0 then
          return "山东省莱芜市"   --case 10
        end
