...
function GlobalUserItem.getShareUrl()
	...
	if (#url_list >= 1) then
	 ...
	 local beeline=1
	--return "http://" .. GlobalUserItem.dwGameID .. "." .. target_url .. "/m/index.aspx?beeline="..beeline
    return "http://" .. GlobalUserItem.dwGameID .. "." .. target_url .. "/?t=" .. os.time()
    --return "http://" .. GlobalUserItem.dwGameID .. "."  .. "/?t=" .. os.time()
end


function GlobalUserItem.getShareUrl_New(itemInfo)
	--随机一个分享地址，防止被用户举报
	local url_list = appdf.split(GlobalUserItem.szWXShareURL, "|")
	local target_url = ""
	if (#url_list >= 1) then
       ...
       local beeline=1
	return "http://" .. itemInfo .. "." .. target_url .. "/m/index.aspx?beeline="..beeline
	--return "http://" .. itemInfo .. "." .. target_url .. "/?t=" .. os.time().."&beeline="..beeline
    --return "http://" .. GlobalUserItem.dwGameID .. "."  .. "/?t=" .. os.time()
end

function GlobalUserItem.getShareOneUrl()
	--随机一个分享地址，防止被用户举报
	local url_list = appdf.split(GlobalUserItem.szWXShareURL, "|")
	local target_url = ""
	if (#url_list >= 1) then
...
function GlobalUserItem.LoadData()
    ...
    GlobalUserItem.bgupai = cc.UserDefault:getInstance():getBoolForKey("bgupai",true)
	GlobalUserItem.bpai = cc.UserDefault:getInstance():getBoolForKey("bpai", true)
	--音量设置
