local BaseMediator = appdf.req("client.src.view.mediator.BaseMediator")
local ExternalFun = appdf.req(appdf.EXTERNAL_SRC .. "ExternalFun")
local ExchangeMediator = class("ExchangeMediator", BaseMediator)
local logincmd = appdf.req(appdf.HEADER_SRC .. "CMD_LogonServer")

function ExchangeMediator:listNotificationInterests()
...
function ExchangeMediator:handleNotification(notification)
...
function ExchangeMediator:onRegister()
...
function ExchangeMediator:onExchangeBankScore(e, score, exchangeType)
...
--function ExchangeMediator:onBindBankInfo(e, realName, bankNo, bankName, bankAddr, alipayID)
function ExchangeMediator:onBindBankInfo(e, BankInfo)
...
function ExchangeMediator:onSubOperateSuccess(pData)
	local lResultCode = pData:readint()
	local szDescribe = pData:readstring()
	self.viewComponent:emit("M2V_ShowToast", szDescribe)
end

function ExchangeMediator:onSubOperateFailure(pData)
	local lResultCode = pData:readint()
	local szDescribe = pData:readstring()
	self.viewComponent:emit("M2V_ShowToast", szDescribe)
end

function ExchangeMediator:onCheckBindBankInfo(e)
	--先读取缓存数据 没有就请求网络数据
...
function ExchangeMediator:onQueryBankInfo(e)
	AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {ctor = {"查询银行信息中..."}, canrepeat = false}, VIEW_LIST.POPWAIT_LAYER)
...
function ExchangeMediator:onSaveBankScore(e, score)
	if (score == 0) then
...
function ExchangeMediator:onTakeBankScore(e, insurepass, score)
	if (score == 0) then
...
function ExchangeMediator:onQueryBankRecord(e)
...
function ExchangeMediator:onQueryExchangeRecord(e)
...
function ExchangeMediator:onSubInsureSuccess(pData)
...
function ExchangeMediator:onSubInsureFailue(pData)
...
--绑定手机
function ExchangeMediator:onBindPhone(e)
