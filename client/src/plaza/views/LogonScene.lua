...
appdf.req(appdf.CLIENT_SRC.."app.models.bit")
...
function LogonScene.onceExcute()
	...
	LogAsset:getInstance():logData(cjson.encode("LogonScene.onceExcute"),true)
	...
	appdf.onSendData(yl.LOGONSERVER, yl.LOGONPORT, QueryData, function (Datatable, Responce)
		...
		if (Responce.code == 0) then
			...
			elseif (Datatable.sub == logincmd.SUB_GP_OPERATE_FAILURE) then
				...
				if self and self._logonFrame then 
					self._logonFrame:onLogonByVisitor()
				end
			...
			local runScene = cc.Director:getInstance():getRunningScene()
--			showToast(runScene, "网络连接失败1111！", 1)
		--	dump(yl.SERVER_LIST[yl.CURRENT_INDEX])
			if  yl.newServerList and nil ~= yl.newServerList[yl.CURRENT_INDEX] then
			--dump(yl.newServerList)
				-- print("yl.CURRENT_INDEX")
				-- dump(yl.CURRENT_INDEX)
				yl.CURRENT_INDEX = yl.CURRENT_INDEX+1
				if yl.newServerList and yl.CURRENT_INDEX > #yl.newServerList then 
					yl.CURRENT_INDEX =1
				end
				if yl.newServerList then 
					yl.LOGONSERVER = yl.newServerList[yl.CURRENT_INDEX]["domain"]
					yl.LOGONPORT = yl.newServerList[yl.CURRENT_INDEX]["port"]
					yl.TOTAL_COUNT = #yl.newServerList
				else
...
function LogonScene:onLogonCallBack(result,message)
	...
	if result == 1 then --成功
		...
		LogAsset:getInstance():logData(cjson.encode("onLogonCallBack"),true)
		LogAsset:getInstance():logData(cjson.encode(BaseConfig.WEB_HTTP_URL),true)
