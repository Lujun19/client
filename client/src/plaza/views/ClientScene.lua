...
local ZhuanPanLayer = import(".layer.plaza.ZhuanPanLayer")
...
ClientScene.BT_VIP		= 31
ClientScene.BT_KEFU2 = 32
ClientScene.BT_KEFU_CLOSE = 33
ClientScene.BT_BIND = 34
ClientScene.BT_BIND_CLOSE = 35
ClientScene.BT_ZHUANPAN = 36
...
if (not BaseConfig.BASE_C_BINVERSION or BaseConfig.BASE_C_BINVERSION == 1) then
	BaseConfig.ONLY_APPSTORE_GAME = {390, 391, 392, 393, 394, 395, 396, 397, 398, 399, 400}
end
function ClientScene:getAniByFile(fileName, armaName, animName, pos, parent)
	ccs.ArmatureDataManager:getInstance():addArmatureFileInfo(fileName)
	local armature = ccs.Armature:create(armaName)
	armature:getAnimation():play(animName)
...
function ClientScene:ctor()
	...
	if (appConfigProxy._appStoreSwitch == 0) then
		csbPath = "plaza/PlazzLayerNew.csb"
	else
	...
	for _,v in ipairs(btKefu) do
		v:setTag(ClientScene.BT_KEFU2)
		v:addTouchEventListener(btcallback)
	end

	local btBind = ExternalFun.seekWigetByNameEx(csbNode, "btn_shoujirenzheng")
	for _,v in ipairs(btBind) do
		v:setTag(ClientScene.BT_BIND)
		v:addTouchEventListener(btcallback)
	end

	local btKefu = ExternalFun.seekWigetByNameEx(csbNode, "btn_zhuanpan")
	for _,v in ipairs(btKefu) do
		v:setTag(ClientScene.BT_ZHUANPAN)
		v:addTouchEventListener(btcallback)
	end
	...
	-- 	--约牌
	-- local btpribrand = ExternalFun.seekWigetByNameEx(csbNode, "btn_kefu")
	-- for _,v in ipairs(btpribrand) do
	...
	-- ClientScene.BT_KEFU_CLOSE


	local btKeFuClose = csbNode:getChildByName("KeFu"):getChildByName("Button_Close");
	btKeFuClose:setTag(ClientScene.BT_KEFU_CLOSE)
	btKeFuClose:addTouchEventListener(btcallback)


	-- local btKeFuClose = csbNode:getChildByName("Bind"):getChildByName("btn_close");
	-- btKeFuClose:setTag(ClientScene.BT_BIND_CLOSE)
	-- btKeFuClose:addTouchEventListener(btcallback)
	...
	for _,v in ipairs(btPersonInfo) do
        ...
        local head = HeadSprite:createNormal(GlobalUserItem, 72)
		head:setCascadeOpacityEnabled(true)
		if nil ~= head then
			local personBtnContentSize = v:getContentSize()
			head:setPosition(38, personBtnContentSize.height / 2)
			v:addChild(head)
            head:setZOrder(2)

            --head:setScale(0.5)
--           local  defaultCell = cc.Sprite:create("plaza/head_icon.png")
...
--        	v:addChild(defaultCell)
       ...
       if GlobalUserItem.cbMemberOrder~=0 then
        ...
        img_level:setPosition(head_icon_size.width+60+180, head_icon_size.height+78)
   ...
   --      img_level:setPosition(head_icon_size.width+75+60, head_icon_size.height+76)
   --      img_level:setScale(2.8)
        -- head:addChild(img_level)
        end
		self:attach("M2V_Update_UserFace", function (e, obj)
			head:updateHead(obj,95)
		end)
	...
	--等级信息
	self.text_level = ExternalFun.seekWigetByNameEx(csbNode, "atlas_levels")
   -- self._level=text_level
	for _,v in ipairs(self.text_level) do
		v:setString(GlobalUserItem.wCurrLevelID.."")
	end

    --进度条
	self.levelpro = ExternalFun.seekWigetByNameEx(csbNode, "bar_progress")
	for _,v in ipairs(self.levelpro) do
		v:setPercent(0)
	end
    ...
    --用户姓名
	local text_name = ExternalFun.seekWigetByNameEx(csbNode, "text_name")
	for _,v in ipairs(text_name) do
		local str = GlobalUserItem.szNickName
		v:setString(" "..str)
	end
	...
	local btn_exit = ExternalFun.seekWigetByNameEx(csbNode, "btn_exit")
	for _,v in ipairs(btn_exit) do
		v:setTag(ClientScene.BT_EXIT)
		v:addTouchEventListener(btcallback)
	...
	local btn_shop_2 = csbNode:getChildByName("btn_shop")
	self:getAniByFile2("animal/shop/skeleton.json", "animal/shop/skeleton.atlas", "Animation1", cc.p(85,0),btn_shop_2)
    ...
    --vip
	local btn_vip = ExternalFun.seekWigetByNameEx(csbNode, "btn_vip")



	for _,v in ipairs(btn_vip) do
		v:setTag(ClientScene.BT_VIP)
	v:addTouchEventListener(btcallback)
	end
    ...
    ccs.ArmatureDataManager:getInstance():addArmatureFileInfo("plaza/JingqinqidaiAnimation.ExportJson")

   -- self:getAniByFile("JingqinqidaiAnimation.ExportJson", "JingqinqidaiAnimation", "JingqinqidaiAnimation", cc.p(100,100), self._csbNode)
...
function ClientScene:showSignPage()
	-- body
	local _checkinlayer = Checkin:create(self)
	self._csbNode:addChild(_checkinlayer)
end

function ClientScene:initGameView( )
	-- body
	local mediumLayout = self._csbNode:getChildByName("medium_layout")
	local hallscrollview = mediumLayout:getChildByName("GameListView")
	hallscrollview:setScrollBarEnabled(false)
	self.hallscrollview = hallscrollview
	local radioGroupLayout = self.hallscrollview:getChildByName("PageRadioGroup")
	local radioButtonGroup = ccui.RadioButtonGroup:create()
    radioButtonGroup:setAllowedNoSelection(false)
    radioGroupLayout:addChild(radioButtonGroup)
    local pageView = self.hallscrollview:getChildByName("PageView")
	--pageView:setScale(0.8)
	local Panel_1 = pageView:getChildByName("Panel_1")
	local Panel_2 = pageView:getChildByName("Panel_2")
    local shareurl = GlobalUserItem.getShareUrl()
	local img_erwei = Panel_1:getChildByName("bt_ads"):getChildByName("img_erwei");
	local _qrcode = QrNode:createQrNode(shareurl, 147, 5, 1)
	_qrcode:setPosition(img_erwei:getPosition())
	 Panel_1:getChildByName("bt_ads"):addChild(_qrcode)
	local shareurl = GlobalUserItem.getShareUrl()
	local img_erwei = Panel_2:getChildByName("bt_ads"):getChildByName("img_erwei");
	local _qrcode = QrNode:createQrNode(shareurl, 145, 5, 1)
	_qrcode:setPosition(img_erwei:getPosition())
	Panel_2:getChildByName("bt_ads"):addChild(_qrcode)

	--设置小点点的位置
	local pageCount = 2
	local radioGroupLayoutSize = radioGroupLayout:getContentSize()
	if (pageCount == 1) then
		local radioBtn = ccui.RadioButton:create("plaza/new/select_2.png","plaza/new/select_1.png", ccui.TextureResType.localType)
	...
	elseif  (pageCount == 2) then
		local radioBtn = ccui.RadioButton:create("plaza/new/select_2.png","plaza/new/select_1.png", ccui.TextureResType.localType)
		radioBtn:move(radioGroupLayoutSize.width / 2-20, radioGroupLayoutSize.height / 2)
			:addTo(radioGroupLayout)
		radioButtonGroup:addRadioButton(radioBtn)
			local radioBtn = ccui.RadioButton:create("plaza/new/select_2.png","plaza/new/select_1.png", ccui.TextureResType.localType)
		radioBtn:move(radioGroupLayoutSize.width / 2+20, radioGroupLayoutSize.height / 2)
 ...
 --    local shareurl = GlobalUserItem.getShareUrl()
	-- local img_erwei = hallscrollview:getChildByName("bt_ads"):getChildByName("img_erwei");
	-- self._qrcode = QrNode:createQrNode(shareurl, 145, 5, 1)
	-- self._qrcode:setPosition(img_erwei:getPosition())
 --    hallscrollview:getChildByName("bt_ads"):addChild(self._qrcode)
 --    hallscrollview:getChildByName("bt_ads"):getChildByName("wang_txt"):setString(GlobalUserItem.getShareOneUrl())
	-- LogAsset:getInstance():logData("appConfigProxy._gameList",true)
	-- LogAsset:getInstance():logData(cjson.encode(appConfigProxy._gameList),true)
	-- LogAsset:getInstance():logData("appConfigProxy._gameList",true)
...
function ClientScene:updateGameListView(  )
	-- body
	local gamelist = {}
	...
	if #gamelist <= 6 then 
		self.hallscrollview:setTouchEnabled(false)
	else 
		local scrollviewPageCount = 1
		local width = display.width
		if #gamelist > 6 then 
			local addCount = math.ceil((#gamelist - 6)/2)
			local addWidth = addCount*display.width/4
			width = width + addWidth
		end
	  	local height = self.hallscrollview:getContentSize().height
 	    self.hallscrollview:setInnerContainerSize(CCSize(width, 200))
		self.hallscrollview:setTouchEnabled(true)
	end
	if #gamelist <= 0 then 
		return
	end
	...
	-- --下一页按钮
	-- local mediumLayout = self._csbNode:getChildByName("medium_layout")
	-- mediumLayout:setContentSize(cc.size(mediumLayout:getContentSize().width,520))
	-- local nextBtn = mediumLayout:getChildByName("nextBtn")
	-- local frontBtn = mediumLayout:getChildByName("frontBtn")
	
	-- self.pageIndex = 1
	-- self.pageCount = math.ceil((#self.gamelist - 6)/8)+1
	-- frontBtn:addClickEventListener(handler(self, self.onPageButtonClickedEvent))
	-- nextBtn:addClickEventListener(handler(self, self.onPageButtonClickedEvent))
	for index = 1,#gamelist do 
		local gameinfo  = gamelist[index]
		self:createGameButton(gameinfo,index)
	end

end

function ClientScene:onButtonClickedEvent(sender)
    -- local senderName = sender:getName()
    -- if senderName == "nextBtn" then
    --     self:scrollNextPage()
 
    -- end
end

function ClientScene:pageScrollView(index)
	if index <= 0 then
		return
	end
	
	if index >= self.pageCount then 
		return
	end
...
function ClientScene:createGameButton(gameinfo,index)
	-- body
	local mediumLayout = self._csbNode:getChildByName("medium_layout")
	local mode = mediumLayout:getChildByName("btn_game_mode")

	local btnGame = self.hallscrollview:getChildByName(string.format("btn_game_%d", index ))
	if btnGame  == nil then 
     	btnGame = mode:clone()
     	btnGame:setName(string.format("btn_game_%d", index ))

    	self.hallscrollview:addChild(btnGame)
    	local posY1= 350
    	local posY2= 130
    	posY = posY1
    	local posX = math.ceil(index/2)*display.width/4.2+display.width/8 +80
    	if (index%2) == 0 then 
    		posY =  posY2
    	end
    	btnGame:setPosition(cc.p(posX,posY))
    end
	...
	--local btnGame = pageLayout:getChildByName(string.format("btn_game_%d", index ))
	btnGame:addClickEventListener(handler(self, self.onGameClickEvent))
	local addY = 50
	if index%2 == 0 then 
		addY =30
	end
	btnGame:setScale(1.1)
	--local btnGame = pageLayout:getChildByName(string.format("btn_game_%d", index ))
	btnGame:addClickEventListener(handler(self, self.onGameClickEvent))
	if tonumber(gameinfo._KindID)  == 6 then
	    self:getAniByFile2("animal/zhajinhua/PNG/skeleton.json", "animal/zhajinhua/PNG/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 202 then
	  	self:getAniByFile2("animal/ddzhu/skeleton.json", "animal/ddzhu/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 389 then
	    	self:getAniByFile2("animal/hongzhong/skeleton.json", "animal/hongzhong/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
    elseif tonumber(gameinfo._KindID)  == 407 then
	  	self:getAniByFile2("animal/likui/lkpy.json", "animal/likui/lkpy.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 601 then
	  	self:getAniByFile2("animal/pdk/pdk.json", "animal/pdk/pdk.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 122 then
	   	self:getAniByFile2("animal/baijiale/skeleton.json", "animal/baijiale/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 211 then
	   	self:getAniByFile2("animal/honghei/skeleton.json", "animal/honghei/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 133 then
	   	self:getAniByFile2("animal/longhudou/skeleton.json", "animal/longhudou/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 109 then
	   	self:getAniByFile2("animal/paijiu/skeleton.json", "animal/paijiu/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 28 then
		local addY = 150
		if index%2 == 0 then 
			addY =130
		end
		self:getAniByFile2("animal/qiangzhuangniuniu/qiangzhuangniu.json", "animal/qiangzhuangniuniu/qiangzhuangniu.atlas", "Animation1", cc.p(145,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 105 then
	 	self:getAniByFile2("animal/zhongfabai/skeleton.json", "animal/zhongfabai/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 36 then
	   	self:getAniByFile2("animal/tongbi/skeleton.json", "animal/tongbi/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
		elseif tonumber(gameinfo._KindID)  == 104  then
	   	self:getAniByFile2("animal/tiandi/skeleton.json", "animal/tiandi/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 49  then
	   	self:getAniByFile2("animal/shisanshui/skeleton.json", "animal/shisanshui/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)
	elseif tonumber(gameinfo._KindID)  == 406  then
		local addY = 150
		if index%2 == 0 then 
			addY =130
		end
	        self:getAniByFile2("animal/shuiguolaba/chaojishuiguo.json", "animal/shuiguolaba/chaojishuiguo.atlas", "Animation1", cc.p(150,addY),btnGame)	
   
	elseif tonumber(gameinfo._KindID)  == 302  then
	        self:getAniByFile2("animal/xuezhandaodi/skeleton.json", "animal/xuezhandaodi/skeleton.atlas", "Animation1", cc.p(150,addY),btnGame)	
   
    else
    	print('没有按钮资源~~~~~~~~~~~~~~~~')
    	dump(gameinfo._KindID)
    ...
    --print("clientgameversion",gameinfo._KindID,version)
    	self.tip_download=btnGame:getChildByName("download_tip"):setZOrder(2)

 	local version = versionProxy:getResVersion(gameinfo._KindID)

    --print("clientgameversion",gameinfo._KindID,version)
    if version then
    	self.tip_download:setVisible(false)
        if gameinfo._ServerResVersion > version then
            if (appConfigProxy._appStoreSwitch == 0) then
                btnGame:getChildByName("update_tips"):setVisible(true)
											btnGame:getChildByName("update_tips"):setZOrder(2);
            else
        ...
        self.tip_download:setVisible(true)
        btnGame:getChildByName("update_tips"):setVisible(false)
    end
...
function ClientScene:initBindPhoneUI()
	--玩家信息页

	local bg = self._csbNode:getChildByName("Bind");
	...
	self._btnBindPhone = self._playerinfoBind:getChildByName("btn_bind_phone")
		self._btnBindPhone:setPressedActionEnabled(true)
	...
	self._btncode = self._playerinfoBind:getChildByName("btn_get_verify_code")
		self._btncode:setPressedActionEnabled(true)
	self._btncode:addClickEventListener(handler(self, self.onButtonClickedEvent))
...
function ClientScene:initGameList()
    ...
    local shareurl = GlobalUserItem.getShareUrl()
		local img_erwei = pageViewTouchEventMask:getChildByName("bt_ads"):getChildByName("img_erwei");
	self._qrcode = QrNode:createQrNode(shareurl, 145, 5, 1)
	self._qrcode:setPosition(img_erwei:getPosition())
    pageViewTouchEventMask:getChildByName("bt_ads"):addChild(self._qrcode)
    pageViewTouchEventMask:getChildByName("bt_ads"):getChildByName("wang_txt"):setString(GlobalUserItem.getShareOneUrl())
    local save_qrcode = pageViewTouchEventMask:getChildByName("bt_ads"):getChildByName("btn_fz")
    save_qrcode:setPressedActionEnabled(true)


	-- save_qrcode:addClickEventListener(function()
	--
  ...
  --   end)
	...
	for i = 1, #backGameList do
		...
		if tonumber(gameinfo._KindID)  == 6 then
    	    self:getAniByFile2("animal/zhajinhua/PNG/skeleton.json", "animal/zhajinhua/PNG/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
    	elseif tonumber(gameinfo._KindID)  == 202 or tonumber(gameinfo._KindID)  == 200 then
          	self:getAniByFile2("animal/ddzhu/skeleton.json", "animal/ddzhu/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
					elseif tonumber(gameinfo._KindID)  == 389 then
            	self:getAniByFile2("animal/hongzhong/skeleton.json", "animal/hongzhong/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 407 then
          	self:getAniByFile2("animal/likui/lkpy.json", "animal/likui/lkpy.atlas", "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 601 then
          	self:getAniByFile2("animal/pdk/pdk.json", "animal/pdk/pdk.atlas", "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 122 then
           	self:getAniByFile2("animal/baijiale/skeleton.json", "animal/baijiale/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 211 then
           	self:getAniByFile2("animal/honghei/skeleton.json", "animal/honghei/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 133 then
           	self:getAniByFile2("animal/longhudou/skeleton.json", "animal/longhudou/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 109 then
           	self:getAniByFile2("animal/paijiu/skeleton.json", "animal/paijiu/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 28 then
						self:getAniByFile2("animal/qiangzhuangniuniu/qiangzhuangniu.json", "animal/qiangzhuangniuniu/qiangzhuangniu.atlas", "Animation1", cc.p(145,113),btnGame)
						-- self:getAniByFile3(unsel_img, sel_img, "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 105 then
         	self:getAniByFile2("animal/zhongfabai/skeleton.json", "animal/zhongfabai/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 36 then
           	self:getAniByFile2("animal/tongbi/skeleton.json", "animal/tongbi/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
      	elseif tonumber(gameinfo._KindID)  == 104  then
           	self:getAniByFile2("animal/tiandi/skeleton.json", "animal/tiandi/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
        elseif tonumber(gameinfo._KindID)  == 49  then
           	self:getAniByFile2("animal/shisanshui/skeleton.json", "animal/shisanshui/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)
    	
		elseif tonumber(gameinfo._KindID)  == 302  then
	        self:getAniByFile2("animal/xuezhandaodi/skeleton.json", "animal/xuezhandaodi/skeleton.atlas", "Animation1", cc.p(150,15),btnGame)	
...
function ClientScene:onGameClickEvent(sender)
	...
	sender:getChildByName("download_tip"):setVisible(false)
...
function ClientScene:onViewLoad()
	...
	appdf.loadImage("client/res/plaza/gamelist.plist","client/res/plaza/gamelist.png", function ()
		printf("bank load finished\n")
	end)
...
--获取互动动画
function ClientScene:getAniByFile2(fileName, armaName, animName, pos, parent)
	-- ccs.ArmatureDataManager:getInstance():addArmatureFileInfo(fileName)
	-- local armature = ccs.Armature:create(armaName)
	-- armature:setPosition(pos)
		-- parent:re,pve
		local ani = parent:getChildByTag(60);
		if ani~= nil then
			ani:removeFromParent();
		end
	    local spine = sp.SkeletonAnimation:create(fileName, armaName,1);
		   ...
		   spine.timeScale = 0.5;
		   	...
		   	spine:setScale(1)
		   		spine:setTag(60);
	    parent:addChild(spine, 1)


	return spine
end

function ClientScene:getAniByFile3(fileName, armaName, animName, pos, parent)
		...
		if ani~= nil then
			...
			local spine = cc.Sprite:create("plaza/plist_gamelist/"..fileName)
			 spine:setPosition(pos)
...
function ClientScene:onButtonClickedEvent(tag,ref)
	...
	if tag == ClientScene.BT_EXIT then
		...
		AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {ctor = {"是否返回登录界面？",function(ok)
			if ok == true then
			   AppFacade:getInstance():sendNotification(GAME_COMMAMD.POP_VIEW, {Name = VIEW_LIST.CLIENT_SCENE})
			end
...
--[[			local _optionlayer = Option:create(self)
		...
		elseif tag == ClientScene.BT_CHECKIN then
			...
			local _checkinlayer = Checkin:create(self)
			self._csbNode:addChild(_checkinlayer)
		elseif tag == ClientScene.BT_KEFU_CLOSE then
			self.KeFuLayer = self._csbNode:getChildByName("KeFu");
		  	self.KeFuLayer:setVisible(false)
		  	if self._kefuWebView  then
		  	 	self._kefuWebView:removeFromParent()		
				self._kefuWebView = nil 
			end
			


		elseif tag == ClientScene.BT_BIND then
			self.BindLayer = self._csbNode:getChildByName("Bind");
		  self.BindLayer:setVisible(true)
		elseif tag == ClientScene.BT_BIND_CLOSE then
			self.BindLayer = self._csbNode:getChildByName("Bind");
			self.BindLayer:setVisible(false)
		elseif tag == ClientScene.BT_ZHUANPAN then

			-- local activityLayer = ActivityLayer:create(self._csbNode)
			-- self._csbNode:addChild(activityLayer)

			self._tablelayer = TableLayer:create(self)
			self._csbNode:addChild(self._tablelayer)

		elseif tag == ClientScene.BT_KEFU2 then
			     print("BT_KEFU2")
			-- AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {}, VIEW_LIST.KEFU_LAYER)
			-- AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {}, VIEW_LIST.KEFU_LAYER)
				local url = "https://chat816.osslines.com/chat/chatClient/chatbox.jsp?companyID=1165740&configID=74694&jid=8887589982&s=1"
			 	if (nil ~= self._kefuWebView) then
					self._kefuWebView:removeFromParent()
				end
			    ...
			    -- end
			 	...
			 	self.KeFuLayer = self._csbNode:getChildByName("KeFu");
			 	local webContent = self.KeFuLayer:getChildByName("Panel_3")
			    self.KeFuLayer:setVisible(true)
				self:showPopWait()
		        self._kefuWebView = ccexp.WebView:create()
				self._kefuWebView:setName("kefuWebView")
				self._kefuWebView:setContentSize(webContent:getContentSize())
		        self._kefuWebView:setPosition(cc.p(webContent:getContentSize().width / 2, webContent:getContentSize().height / 2))
		        self._kefuWebView:setScalesPageToFit(true)

		        self._kefuWebView:loadURL(url)
		      

		        self._kefuWebView:setOnDidFailLoading(function ( sender, url )
		            self:dismissPopWait()
		        
		        end)
		        self._kefuWebView:setOnShouldStartLoading(function(sender, url)  
		            return true
		        end)
		        self._kefuWebView:setOnDidFinishLoading(function(sender, url)
		            self:dismissPopWait()
		        end)
		
			    webContent:addChild(self._kefuWebView)
		...
		elseif tag == ClientScene.BT_RECHARGE then
			...
			-- else
			-- 	AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {ctor = {3}}, VIEW_LIST.DUIHUAN_LAYER)
			-- end
		...
		elseif tag == ClientScene.BT_BIND_MOBILE then
		...
		elseif tag == ClientScene.BT_EXIT then
			AppFacade:getInstance():sendNotification(GAME_COMMAMD.POP_VIEW, {Name = VIEW_LIST.CLIENT_SCENE})
		...
		elseif tag == ClientScene.BT_GONGGAO then
            ...
            elseif tag == ClientScene.BT_VIP then
				AppFacade:getInstance():sendNotification(GAME_COMMAMD.PUSH_VIEW, {}, VIEW_LIST.VIP_LAYER)
...
function ClientScene:closeTable(nTag, param)
	 self._tablelayer:removeFromParent();
end
...
function ClientScene:getTagLayer(tag, param)
	...
	elseif tag == yl.ZHUANPAN then
		dst = ZhuanPanLayer:create(self)
