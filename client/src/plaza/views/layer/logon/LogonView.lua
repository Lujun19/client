...
LogonView.BT_LOGON2 = 11
LogonView.BT_LOGON_CLOSE = 12
LogonView.BT_KEFU = 13
LogonView.BT_KEFU_CLOSE = 14
...
function LogonView:ctor(serverConfig, viewBase)
	...
	self._csbNode = csbNode
		...
		-- -- 	 self._netWaitAni:play("chupai", true)
		-- -- 	 self._netWaitAni:clearLastFrameCallFunc()
		-- -- end)
		--
		-- self:playAnimal("nan1","zhuapaikaipai");
		--
		-- local csbPath = "renwu/nv1.csb"
		-- self._netWait = ExternalFun.loadCSB(csbPath, self)
		-- self._netWait:setPosition(yl.WIDTH / 2, yl.HEIGHT / 2)
		-- self._netWaitAni = ExternalFun.loadTimeLine(csbPath)
		-- self._netWait:runAction(self._netWaitAni)
		...
		-- -- end)
		--
		-- self:playAnimal("nv1","chuapaikaipai");
	...
	self.logonAccountLayer = self._csbNode:getChildByName("LoginLayer_zhanghaologin");
	self.logonAccountLayer:setVisible(false);



	local text_field_bg_1 = self.logonAccountLayer:getChildByName("account_bg")
	...
	local text_field_bg_2 = self.logonAccountLayer:getChildByName("pass_bg")
	...
	--记住密码
	self._rememberPass = self.logonAccountLayer:getChildByName("cbtRememberPass")
	self._rememberPass:setTag(LogonView.CBT_RECORD)
	self._rememberPass:addTouchEventListener(btcallback)

	self._accountClose = self.logonAccountLayer:getChildByName("Button_close")
	self._accountClose:setTag(LogonView.BT_LOGON_CLOSE)
	self._accountClose:addTouchEventListener(btcallback)
	...
	local btnAccountsLogon2 = self.logonAccountLayer:getChildByName("logon_button_logon")
		:setTag(LogonView.BT_LOGON2)
	btnAccountsLogon2:setPressedActionEnabled(true)
	btnAccountsLogon2:addTouchEventListener(btcallback)
	...
	local registerLogon = self.logonAccountLayer:getChildByName("btnRemember")
	...
	local btnVisitorsLogon = self._csbNode:getChildByName("btnWexin")
		btnVisitorsLogon:setPressedActionEnabled(true)
		:setTag(LogonView.BT_WECHAT)
		:setName("btn_3")
	btnVisitorsLogon:addTouchEventListener(btcallback)

	local btnKeFu = self._csbNode:getChildByName("btn_kefu")
	btnKeFu:setPressedActionEnabled(true)
		:setTag(LogonView.BT_KEFU)
		:setName("btn_8")
	btnKeFu:addTouchEventListener(btcallback)

	local btKeFuClose = self._csbNode:getChildByName("KeFu"):getChildByName("Button_Close");
	btKeFuClose:setTag(LogonView.BT_KEFU_CLOSE)
...
function LogonView:playAnimal( ren, action )
		self._netWait:getChildByName("buyao"):setVisible(false);
		self._netWait:getChildByName("chupai"):setVisible(false);
		self._netWait:getChildByName("wait"):setVisible(false);
		self._netWait:getChildByName("chupaikaishi"):setVisible(false);
		self._netWait:getChildByName(action):setVisible(true);
		self._netWaitAni:play(action, false)
		self._netWaitAni:setLastFrameCallFunc(function()
			 self._netWaitAni:play(action, true)
			 self._netWaitAni:clearLastFrameCallFunc()
		end)
...
function LogonView:onButtonClickedEvent(tag,ref)
	...
	elseif tag == LogonView.BT_VISITOR then
       ...
       -- showToast(self, "注册联系推广员" , 2)
    elseif tag == LogonView.BT_KEFU then
     ...
     elseif tag == LogonView.BT_KEFU_CLOSE then
  	...
  	elseif tag == LogonView.BT_LOGON2 then
	...
	elseif tag == LogonView.BT_LOGON_CLOSE then
			self.logonAccountLayer:setVisible(false);
	elseif tag == LogonView.BT_LOGON then



		self.logonAccountLayer:setVisible(true);
