...
function GameChatLayer:getTextChatSize(str)
	local tmp = cc.LabelTTF:create(str, "base/fonts/round_body.ttf", SIZE_CHAT_LABEL, cc.size(LEN_CHAT_WIDTH,0))
...
function ChatItem:refreshTextItem(str,sendusernick)
	if nil == self.m_labelChat then
		self.m_labelChat = cc.LabelTTF:create(str, "base/fonts/round_body.ttf", SIZE_CHAT_LABEL, cc.size(LEN_CHAT_WIDTH,0), cc.TEXT_ALIGNMENT_LEFT)
...
function ChatItem:refreshRecordItem(str,sendusernick)
    if nil == self.m_labelChat then
        self.m_labelChat = cc.LabelTTF:create(str, "base/fonts/round_body.ttf", 20, cc.size(LEN_CHAT_WIDTH,0), cc.TEXT_ALIGNMENT_LEFT)
...
function ChatItem:addSendUser(posHeight,sendusernick)
	if nil ~= sendusernick and type(sendusernick) == "string" then
		if nil == self.m_clipUserName then
			self.m_clipUserName = ClipText:createClipText(cc.size(200,20), sendusernick .. ":", "base/fonts/round_body.ttf", 20)
