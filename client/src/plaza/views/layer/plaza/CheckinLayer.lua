...
function CheckinLayer:ctor(scene)
	...
	--加载csb资源
	local rootLayer, csbNode = ExternalFun.loadRootCSB("Checkin/CheckinLayer.csb", self)
	self._rootLayer = rootLayer
	self.m_checkinBg = csbNode:getChildByName("checkin_bg_xg")
	...
	--[[frame = cc.SpriteFrameCache:getInstance():getSpriteFrame("checkin_bg.png")
	if nil ~= frame then
		local sp = cc.Sprite:createWithSpriteFrame(frame)
	...
	--签到背景效果
	display.newSprite("Checkin/checkin_bg_xg.png")
	:move(yl.WIDTH/2,yl.HEIGHT/2)
	:addTo(self)

	display.newSprite("Checkin/title_checkin.png")
	:move(yl.WIDTH/2,yl.HEIGHT/2 + 215)
	:addTo(self)
	...
	--[[local btn = ccui.Button:create("public/p_bt_close_0.png","public/p_bt_close_1.png")
	:move(yl.WIDTH/2+450,yl.HEIGHT/2 + 215)
	:addTo(self)
	...
	-- 退出按钮
	self._return = self.m_checkinBg:getChildByName("Button_return")
	...
	-- --灯背景效果--MXM
 --    display.m_actLightBg = ExternalFun.loadTimeLine( "Checkin/Checkin_Light_Bg.csb")
 --    local ani_lightbg = ExternalFun.loadCSB("Checkin/Checkin_Light_Bg.csb",self.m_checkinBg)
 --            :setPosition(self.m_checkinBg:getPosition())
 --    display.m_ani_lightbg = ani_lightbg

 --    ExternalFun.SAFE_RETAIN(display.m_actLightBg)
 --    display.m_ani_lightbg:stopAllActions()
 --    display.m_actLightBg:gotoFrameAndPlay(0,true)
 --    display.m_ani_lightbg:runAction(display.m_actLightBg)

	--灯效果--MXM
	-- self._light = self.m_checkinBg:getChildByName("light")
  --   display.m_actLight = ExternalFun.loadTimeLine( "Checkin/Checkin_Light.csb")
  --   local ani_light = ExternalFun.loadCSB("Checkin/Checkin_Light.csb",self.m_checkinBg)
  --           :setPosition(self._light:getPosition())
  --   display.m_ani_light = ani_light
	--
  --   ExternalFun.SAFE_RETAIN(display.m_actLight)
  --   display.m_ani_light:stopAllActions()
  --   display.m_actLight:gotoFrameAndPlay(0,true)
  --   display.m_ani_light:runAction(display.m_actLight)
	...
	self._btConfig:setVisible(GlobalUserItem.wSeriesDate == 1)

	self._today_btConfig = self.m_checkinBg:getChildByName("Button_checkin_today")
	self._today_btConfig:setVisible(GlobalUserItem.wSeriesDate == 0)
	...
	for i=0,CheckinLayer.CHECKIN_NUMBER-1 do
		...
		if i<4 then
			...
			--金币标志
			display.newSprite("Checkin/bt_golden_box.png")
			:move(370+200*i,520)
			...
			--金币数字
			cc.LabelAtlas:_create("1000", "Checkin/num_checkin_3.png", 12, 16, string.byte("."))
			:move(370+200*i,450)
			:setAnchorPoint(cc.p(0.5,0.5))
		   ...
		   -- :setVisible(false)
			...
			--天数背景
			display.newSprite("Checkin/text_checkin_1.png")
			:move(178+220*i-600,207)
			:setTag(CheckinLayer.TEXT_DAY+i)
			:addTo(self)

			--天数数字
			cc.LabelAtlas:_create(""..i+1, "Checkin/num_checkin_1.png", 19, 25, string.byte("0"))
			:move(178+220*i-600,210)
			:setAnchorPoint(cc.p(0.5,0.5))
			:setTag(CheckinLayer.NUM_DAY+i)
			:addTo(self)
...
function CheckinLayer:onButtonClickedEvent(tag, ref)
...
function CheckinLayer:onCheckinCallBack(result, message, subMessage)
	...
	if self._checkinFrame.GETMEMBERGIFT == result then
		...
		if GlobalUserItem.bTodayChecked == true then
			ExternalFun.enableBtn(self._btConfig, false)
			self._btConfig:setVisible(false)
			self._today_btConfig:setVisible(true)
			-- self._btConfig:setVisible(true)
		else
			ExternalFun.enableBtn(self._btConfig, true)
			self._btConfig:setVisible(true)
			self._today_btConfig:setVisible(false)
		end
...
function CheckinLayer:reloadCheckin()
	...
	if nil ~= todayCheck and GlobalUserItem.bTodayChecked then
		todayCheck:setTexture("Checkin/back_checkin_2.png")
	end
	...
	if GlobalUserItem.wSeriesDate ~= 0 then
		for i=0,GlobalUserItem.wSeriesDate-1 do
			...
			self:getChildByTag(CheckinLayer.ICON_GOLD+i):setTexture("Checkin/bt_golden_box.png")
	...
	for i=0,CheckinLayer.CHECKIN_NUMBER-1 do
		...
		self:getChildByTag(CheckinLayer.TEXT_GOLD+i):setString("".. string.formatNumberFhousands(reword) .."")
	end
