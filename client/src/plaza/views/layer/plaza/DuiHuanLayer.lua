...
local DuiHuanLayer = class("DuiHuanLayer", cc.BaseLayer)

local ExternalFun = appdf.req(appdf.EXTERNAL_SRC .. "ExternalFun")
local ClipText = appdf.req(appdf.EXTERNAL_SRC .. "ClipText")
local MultiPlatform = appdf.req(appdf.EXTERNAL_SRC .. "MultiPlatform")

function DuiHuanLayer:ctor(pageindex)
	self.super.ctor(self)

	self._pageDefaultIndex = 3
    -- 加载csb资源
	local csbPath = "DuiHuan/BankLayer.csb"
...
function DuiHuanLayer:initUI()
	...
	self.m_rtoBankExchange = ccui.RadioButton:create("exchange_btn_1.png","exchange_btn_1.png","exchange_btn_0.png","exchange_btn_0.png","exchange_btn_1.png", ccui.TextureResType.plistType)
	...
	self.m_rtoBankExchangeRecord = ccui.RadioButton:create("exchange_btn_record_1.png","exchange_btn_record_1.png","exchange_btn_record_0.png","exchange_btn_record_0.png","exchange_btn_record_1.png", ccui.TextureResType.plistType)
	...
	elseif (self._pageDefaultIndex == 3) then
		...
		if (GlobalUserItem.BankInfoData ~= nil and GlobalUserItem.BankInfoData.szBankNo == "" and  GlobalUserItem.BankInfoData.szAlipayID == "") then
...
--初始化存款界面
function DuiHuanLayer:initSaveUI()
	local bg = self._csbNode:getChildByName("bg")
    --游戏币存
...
--初始化取款界面
function DuiHuanLayer:initTakeUI()
	--游戏币取
	local bg = self._csbNode:getChildByName("bg")
...
--初始化存取款记录界面
function DuiHuanLayer:initSaveTakeRecordUI(data)
	if (not data) then
		return
	end
...
--初始化兑换界面
function DuiHuanLayer:initExchangeUI()
	--游戏币取
	local bg = self._csbNode:getChildByName("bg")
...
function DuiHuanLayer:initExchangeSubBind(parent)
...
function DuiHuanLayer:initExchangeSubExchange(parent, data)
	local layout_sub_exchange = parent:getChildByName("layout_sub_exchange")


	LogAsset:getInstance():logData("initExchangeSubExchange",true);
	LogAsset:getInstance():logData(cjson.encode(data),true);
...
--初始化兑换记录界面
function DuiHuanLayer:initExchangeRecordUI(data)
	if (not data) then
		return
	end
...
function DuiHuanLayer:onWillViewEnter()
...
function DuiHuanLayer:onWillViewExit()
...
function DuiHuanLayer:onEnterTransitionFinish( )
    --if 1 == GlobalUserItem.cbInsureEnabled then
    --   self:showPopWait()
...
function DuiHuanLayer:onFlushBank()
	self:showPopWait()
	self._bankFrame:onFlushBank()
end

--按键监听
function DuiHuanLayer:onButtonClickedEvent(sender)
	ExternalFun.playClickEffect()

	local senderName = sender:getName()

	if (senderName == "Button_return") then
		AppFacade:getInstance():sendNotification(GAME_COMMAMD.POP_VIEW, {Name = VIEW_LIST.DUIHUAN_LAYER})
...
--输入框监听
function DuiHuanLayer:onEditEvent(event,editbox)
	if event == "changed" then
		local src = editbox:getText()
		if src == nil or src == "" then
			return
...
function DuiHuanLayer:onSelectedEvent(sender,eventType)
	local senderName = sender:getName()
...
--取款操作
function DuiHuanLayer:onTakeScore()
...
--存款
function DuiHuanLayer:onSaveScore()
	if GlobalUserItem.cbInsureEnabled==0 then
		self._notifyLayer:setVisible(true)
		return
	end
...
function DuiHuanLayer:onTransferScore()
	if GlobalUserItem.cbInsureEnabled==0 then
		self._notifyLayer:setVisible(true)
		return
	end
...
--获取父场景节点(ClientScene)
function DuiHuanLayer:getRootNode( )
	return self:getParent():getParent():getParent():getParent()
end

--操作结果
function DuiHuanLayer:onBankCallBack(result,message)
...
-- 显示凭证
function DuiHuanLayer:showCerLayer( tabData )
