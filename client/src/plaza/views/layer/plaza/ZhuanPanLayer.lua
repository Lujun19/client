...
local ZhuanPanLayer = class("ZhuanPanLayer", cc.BaseLayer)

-- 进入场景而且过渡动画结束时候触发。
function ZhuanPanLayer:onEnterTransitionFinish()
    return self
end

-- 退出场景而且开始过渡动画时候触发。
function ZhuanPanLayer:onExitTransitionStart()
    return self
end

function ZhuanPanLayer:ctor()
	self.super.ctor(self)

	--加载csb资源
	local csbPath = "activity/ZhuanPanLayer.csb"
...
function ZhuanPanLayer:onWillViewEnter()
	self:initUI()
...
function ZhuanPanLayer:onWillViewExit()
...
--按键监听
function ZhuanPanLayer:onButtonClickedEvent(sender)
