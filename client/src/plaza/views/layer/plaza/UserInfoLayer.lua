...
function UserInfoLayer:ctor(pageIndex)
    ...
    -- 加载csb资源
	local csbPath = "Userinfo/UserInfoScene2.csb"
...
function UserInfoLayer:initUI()
    ...
    self._playerinfoLeft = bg:getChildByName("UserInfoRight")
	self._playerinfoRight = bg:getChildByName("UserInfoRight")
	...
	self.m_rtoBoy = ccui.RadioButton:create("btnUnsel_0.png","btnUnsel_0.png","btnSel_0.png","btnUnsel_0.png","btnUnsel_0.png", ccui.TextureResType.plistType)
	...
	self.m_rtoGirl = ccui.RadioButton:create("btnUnsel_0.png","btnUnsel_0.png","btnSel_0.png","btnUnsel_0.png","btnUnsel_0.png", ccui.TextureResType.plistType)
	...
	self.m_boy = self._playerinfoRight:getChildByName("sex_0");
	self.m_girl = self._playerinfoRight:getChildByName("sex_1");

	--修改账号密码
	self.m_btnModifyAccountPass = self._playerinfoRight:getChildByName("btn_modify_pass")
    self.m_btnModifyAccountPass:setPressedActionEnabled(true)
    ...
    self.m_btnModifyBanktPass:setPressedActionEnabled(true)
    ...
    local info_left_bg_R = bg:getChildByName("Bt_R")
	self.m_btnReturn = self._playerinfoRight:getChildByName("btn_return")
    self.m_btnReturn:setPressedActionEnabled(true)
	self.m_btnReturn:addClickEventListener(handler(self, self.onButtonClickedEvent))



	--个人信息
	local rto_person_container_1 = info_left_bg_R:getChildByName("rto_person_container_1")
	...
	--绑定手机
	local rto_bind_container_2 = info_left_bg_R:getChildByName("rto_bind_container_2")
    ...
    self.m_btnHead:setPressedActionEnabled(true)
	...
	self._sphead = HeadSprite:createClipHead(GlobalUserItem, btnHeadBackMaskSize.width*0.9)
	self._sphead:setPosition(cc.p(btnHeadBackMaskSize.width / 2, btnHeadBackMaskSize.height / 2))
    --self._sphead:setscene(0.9)
	...
	self.m_btnCopy = self._playerinfoLeft:getChildByName("btn_copy")
    self.m_btnCopy:setPressedActionEnabled(true)
	self.m_btnCopy:addClickEventListener(handler(self, self.onButtonClickedEvent))
	...
	self.m_btnMobile = self._playerinfoLeft:getChildByName("btn_mobile")
	self.m_btnMobile:addClickEventListener(handler(self, self.onButtonClickedEvent))
...
function UserInfoLayer:initBindPhoneUI()
	...
	self._btnBindPhoneClose = self._playerinfoBind:getChildByName("btn_bind_return")
  self._btnBindPhoneClose:setPressedActionEnabled(true)
	self._btnBindPhoneClose:addClickEventListener(handler(self, self.onButtonClickedEvent))
...
function UserInfoLayer:onWillViewEnter()
	LogAsset:getInstance():logData(cjson.encode("onWillViewEnter"),true)
...
function UserInfoLayer:onWillViewExit()
	LogAsset:getInstance():logData(cjson.encode("onWillViewExit"),true)
	...
	if (self.m_rtoPsersonInfo:isSelected()) then
		...
		LogAsset:getInstance():logData(cjson.encode("exitViewUserInfo"),true)
...
function UserInfoLayer:onButtonClickedEvent(sender)
	...
	elseif (senderName == "btn_bind_return") then
			AppFacade:getInstance():sendNotification(GAME_COMMAMD.POP_VIEW, {Name = VIEW_LIST.PERSON_LAYER})
	elseif (senderName == "btn_copy") then
		MultiPlatform:getInstance():copyToClipboard(GlobalUserItem.dwGameID .. "")
	...
	elseif (senderName == "btn_mobile") then
		if (GlobalUserItem.szRegisterMobile == "") then
			self._radioTopButtonGroup:setSelectedButton(self.m_rtoBindPhone)
			AppFacade:getInstance():sendNotification(GAME_COMMAMD.PLAY_ONE_EFFECT, {}, "sound/first_bind.mp3")
			self._csbNodeAni:play("showBindPhone", false)
