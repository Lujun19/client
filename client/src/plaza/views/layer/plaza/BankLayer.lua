...
function BankLayer:ctor(pageindex)
	...
	self._pageDefaultIndex = 0;
    -- 加载csb资源
	local csbPath = "Bank/BankLayer2.csb"
...
function BankLayer:initUI()
    ...
    --携带金币
    self._txtScore = self._takeNode:getChildByName("content"):getChildByName("atlas_coin")
		-- self._txtScore2 = self._saveNode:getChildByName("content"):getChildByName("atlas_coin")
		self._txtScore2 = self._saveNode:getChildByName("content"):getChildByName("atlas_coin")
	...
	self:attach("M2V_Update_UserScore", function (e, score)
		...
		elseif (score >= 100000000) then
			self._txtScore:setString(str .. "亿")
			self._txtScore2:setString(str .. "亿")
		end

		-- self._txtScore:setString(tostring(score))
		-- self._txtScore2:setString(tostring(score))
    ...
    -- self._txtInsure = top_bg:getChildByName("atlas_bankscore")
		self._txtInsure = self._takeNode:getChildByName("content"):getChildByName("atlas_bankscore")
		self._txtInsure2 = self._saveNode:getChildByName("content"):getChildByName("atlas_bankscore")
	...
	-- self._radioLeftMenuButtonGroup:setVisible(false);
...
function BankLayer:sliderButtonEvent(sender,eventType)
	if GlobalUserItem.lUserScore <= 50 then
		self.edit_save:setEnabled(false)
		self.slider:setEnabled(false)
		self.slider:setPercent(0)
		self.edit_save:setText("0")
		showToast(cc.Director:getInstance():getRunningScene(), "使用银行库存金币，需随身金币大于50！", 1)
		return
	end
	    self.slider:setEnabled(true)
		if eventType == ccui.SliderEventType.percentChanged then
		local parcent = self.slider:getPercent()
		local SliUserScore = string.format("%0.2f", GlobalUserItem.lUserScore * (parcent/100))
		self.edit_save:setText(SliUserScore)
		end 
end
...
function BankLayer:initSaveUI()
    ...
    --滑动条
    local slider = self._saveNode:getChildByName("Slider_1")
	self.slider = slider
	self.slider:addEventListener(handler(self,self.sliderButtonEvent))
	...
	local function onEditEvent(event,editbox)
		...
		elseif event == "return" then
			...
			if type(ndst) == "number" and ndst < 9999999999999 then
				...
				local sliderNum = dst2f/GlobalUserItem.lUserScore*100
				self.slider:setPercent(sliderNum)
			else
				self.text_format_save_score:setString("")
			end
 ...
 --    btn_save_10:setPressedActionEnabled(true)
 ...
 --    btn_save_100:setPressedActionEnabled(true)
 ...
 --    btn_save_1000:setPressedActionEnabled(true)
 ...
 --    btn_save_10000:setPressedActionEnabled(true)
...
--初始化存款界面
function BankLayer:initGiftUI()
	local bg = self._csbNode:getChildByName("bg")
    --游戏币存
    self._giftNode = bg:getChildByName("layout_gift")

	local text_field_bg_1 =  self._giftNode:getChildByName("text_field_bg_1")
	local text_field_1 =  text_field_bg_1:getChildByName("text_field_1")
	local text_field_size = text_field_1:getContentSize()
	--金额输入
	self.edit_gift_uid = ccui.EditBox:create(text_field_size, ccui.Scale9Sprite:create("public/text_field_space.png"))
		:move(text_field_size.width / 2, text_field_size.height / 2)
		:setFontName("base/fonts/round_body.ttf")
		:setPlaceholderFontName("base/fonts/round_body.ttf")
		:setFontSize(32)
		...
		local text_field_bg_2 =  self._giftNode:getChildByName("text_field_bg_2")
		local text_field_2 =  text_field_bg_2:getChildByName("text_field_2")
		local text_field_size = text_field_2:getContentSize()

		self.edit_gift_money = ccui.EditBox:create(text_field_size, ccui.Scale9Sprite:create("public/text_field_space.png"))
			:move(text_field_size.width / 2, text_field_size.height / 2)
			:setFontName("base/fonts/round_body.ttf")
			:setPlaceholderFontName("base/fonts/round_body.ttf")
			:setFontSize(32)
...
function BankLayer:initTakeUI()
    ...
    btn_take_10:setPressedActionEnabled(true)
    ...
    btn_take_100:setPressedActionEnabled(true)
    ...
    btn_take_1000:setPressedActionEnabled(true)
    ...
    btn_take_10000:setPressedActionEnabled(true)
...
function BankLayer:initSaveTakeRecordUI(data)
	...
	self._layout_save_take_record = self._csbNode:getChildByName("bg"):getChildByName("layout_save_take_record")
...
function BankLayer:initExchangeSubBind(parent)
    ...
    btn_bind:setPressedActionEnabled(true)
...
function BankLayer:initExchangeSubExchange(parent, data)
	...
	local text_field_1 =  text_field_bg_1:getChildByName("text_field_12")
	local text_field_size = text_field_1:getContentSize()
...
function BankLayer:initExchangeRecordUI(data)
	...
	self._layout_exchange_record = self._csbNode:getChildByName("bg"):getChildByName("layout_exchange_record")
...
function BankLayer:onSelectedEvent(sender,eventType)
	...
	if (ccui.RadioButtonEventType.selected == eventType) then
		...
		elseif (senderName =="rto_bank_save_take_record") then
			...
			-- self
			self._takeNode:setVisible(false);
			self._saveNode:setVisible(false);
			self._exchangeNode:setVisible(false);
			self._layout_exchange_record:setVisible(false);
			self._layout_save_take_record:setVisible(false);
