...
function RoomListLayer:onWillViewEnter()
	...
	LogAsset:getInstance():logData("GlobalUserItem.getCurGameKind()",true)
	LogAsset:getInstance():logData(cjson.encode(GlobalUserItem.getCurGameKind()),true)
	...
	local image = ccui.ImageView:create(frameName)
	image:setVisible(false)
	
	imgViewTitle:addChild(image)
	local size = image:getContentSize()
	imgViewTitle:setContentSize(size)
	...
	for i = 1, #self.m_tabRoomListInfo do
		...
		if 8 == wLv then
            ...
            --end
            local contentSize = btnRoom:getContentSize()
            print("contentSize")
            dump(contentSize)
