...
function TableLayer:ctor(scene)
	...
	--加载csb资源
	local rootLayer, csbNode = ExternalFun.loadRootCSB("table2/TableLayer.csb", self)
	...
	self.m_btnStart2 = csbNode:getChildByName("start_btn3")
	...
	self.m_btnClose = csbNode:getChildByName("close_btn")
	self.m_btnClose:addTouchEventListener(function ( sender, tType )
		if tType == ccui.TouchEventType.ended then
          self._scene:closeTable();
    end
	end)
    ...
    local wechatBtn = csbNode:getChildByName("Button_btn_wechat")
...
function TableLayer:onButtonClickedEvent(sender)
    local senderName = sender:getName()
    if senderName == "Button_btn_wechat" then
        local function sharecall( isok )
            if type(isok) == "string" and isok == "true" then
                showToast(self, "分享完成", 1)
            end
...
function TableLayer:onEnterTransitionFinish( )
	...
	-- 获取抽奖奖品配置
	local url = BaseConfig.WEB_HTTP_URL .. "/WS/Lottery.ashx"
...
function TableLayer:rotateTo( idx )
  ...
  -- local angle = 10800;
	...
	local seq = cc.Sequence:create(easeRotate, call)
	self.m_btnStart2:stopAllActions()
	self.m_btnStart2:runAction(seq)
