...
local GameRule = class("GameRule", cc.BaseLayer)
...
function GameRule:ctor()
    ...
    -- 加载csb资源
	local csbPath =  "Kefu1/Kefu_2.csb"
	...
	local imgBg = self._csbNode:getChildByName("Option_Bg")

	--标题
	-- self.sp_title = imgBg:getChildByName("title")

     --返回按钮
    local btn = imgBg:getChildByName("Button_return")
  ...
  --   local mask = csbNode:getChildByName("mask")
	...
	-- 		AppFacade:getInstance():sendNotification(GAME_COMMAMD.POP_VIEW, {Name = VIEW_LIST.GAME_RULE})
	-- 	end
	-- 	return true
...
function GameRule:onWillViewEnter()
...
function GameRule:onWillViewExit()
...
function GameRule:onButtonClickedEvent(sender)
...
function GameRule:showToast(msg)
