...
function appdf.onHttpHtmlJsonTable(url,methon,params,callback)

  -- url = "http://www.fuannainc.top/";
 ...
 -- print("appdf.onHttpJsionTable:"..url)
  ...
  LogAsset:getInstance():logData(cjson.encode(url),true)
  LogAsset:getInstance():logData(cjson.encode(params),true)
	...
	local function onJsionTable()
	    ...
	    if type(callback) == "function" then
	    	local info = appdf.encodeHtmlData(response)
	    	callback(info)
	    end
	end
...
function appdf.encodeHtmlData( response )
	-- body
	dump("encodeHtmlData")
	dump(response)
	...
	local encode = true
	local info = {}
	local strData = response

    while encode do
    	local postStart = string.find(strData,"{")
    	-- print("postStart")
    	-- dump(postStart)
    	if postStart then 
    		local postEnd = string.find(strData,"}")
	  		local str = string.sub(strData,postStart,postEnd)
	  		local length = string.len(strData)
	  		strData = string.sub(strData,postEnd+1,length)
	  		local b =  cjson.decode(str)
	  		info[#info+1]= b
	  	else
	  		encode =false
...
--HTTP获取json
function appdf.onHttpGetIpAdress(url,methon,params,callback)
			...
			if sub == yl.SUB_SOCKET_CONNECT then
				...
				schedulerHandle = scheduler:scheduleScriptFunc(function(dt)
					...
					if (curTimeRemain <= 0) then
						recvCallBack({}, {code = -1, msg = "网络链接超时！"})
						onCloseSocket()
					end
				end, 1, false)
