...
function PriFrame:sendCreateRoom()
    ...
    buffer:pushdword(GlobalUserItem.getCurGameKind())
...
function PriFrame:onSubSearchRoomResult( pData )
    ...
    if 0 == cmd_table.dwServerID then
		...
		if (cmd_table.lCellScore*cmd_table.lPriRoomMultiple*cmd_table.dwDrawCountLimit) > GlobalUserItem.lUserScore then
			self._callBack(LOGINSERVER(cmd_pri_login.SUB_MB_SEARCH_RESULT), "对不起！进入此房间需要 " .. cmd_table.lCellScore*cmd_table.lPriRoomMultiple*cmd_table.dwDrawCountLimit .. " 金币!")
			return
		end
    end
    -- 信息记录
